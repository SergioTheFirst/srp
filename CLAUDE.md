...
- Git: branch-first → gate green → subagent review → `merge --no-ff` → **`push origin main`** — all AUTOMATIC, never ask to merge/push (user 2026-06-21 `[[auto-merge-push-no-ask]]`); conventional commits, NO attribution. **Auto-commit AND auto-push each important/complete change WITHOUT asking** (user 2026-06-10/06-21). NEVER `git add -A`/sweep — stage ONLY files touched for THIS change; local `client/config.json`/`org_directory.json` never committed (template stays empty).
- **No dormant features (user 2026-06-21 `[[no-disabled-by-default]]`):** anything OFF-by-default in code MUST be ON in shipped `server/config.json` (or dashboard-toggleable) — enable it yourself in the SAME change. Code keeps the secure default + stop-gate; deployment opts in (printers + netdisco are ON in `server/config.json`).
