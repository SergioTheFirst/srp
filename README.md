...
- **Байесовский риск по классам отказа** — накопитель, питание/перегрев,
...
- **Прогноз-траектории**: тренды заполнения диска, деградации ОС, старения ПО;
  движок хранилища с правилами по pending-секторам и рецидивам
...
│   ├── analytics/         движки: диск, деградация ОС, старение ПО, аномалии парка
...
SRP распространяется по лицензии [MIT](LICENSE): свободное использование,
изменение и распространение — при сохранении уведомления об авторстве.

Собранный `srp-agent.exe` включает интерпретатор CPython (лицензия PSF,
совместима с распространением). Серверные зависимости (FastAPI, uvicorn,
pydantic, Jinja2 и др.) не входят в репозиторий — они ставятся из PyPI под
собственными пермиссивными лицензиями (MIT / BSD / Apache-2.0).
