...
def test_archive_processes_groups_independently(tmp_path, monkeypatch) -> None:
    """o5-D10: архиватор читал `SELECT * FROM table WHERE received_at < ?` целиком
    в память — на запущенной базе это все устаревшие сырые строки парка сразу.
    Читать надо группами (устройство × день), так пик памяти ограничен группой."""
    import zlib

    from server import db

    db.init_db(str(tmp_path / "arch.db"))
    for dev in ("a1", "a2", "a3"):
        for day in ("2020-01-01", "2020-01-02"):
            for i in range(3):
                db.store_heartbeat(
                    dev,
                    f"{day}T10:0{i}:00+00:00",
                    {"cpu_pct": 1.0},
                    received_at=f"{day}T10:0{i}:00+00:00",
                )
    ...
    with db._connect() as conn:
        ...
        n = db._archive_aged_rows(_Rec(), "heartbeats", "2021-01-01T00:00:00+00:00")
        conn.commit()

    assert n == 18
    assert calls["n"] == 6, "сжатие вызвано не по одной группе (3 устройства × 2 дня)"
    bulk = [q for q in seen if "SELECT *" in q and "device_id" not in q]
    assert not bulk, f"вся таблица читается одним запросом: {bulk}"
    rows = db.get_raw_archive("a1", "heartbeats", 36500)
    assert len(rows) == 6


def test_received_at_indexes_exist_after_legacy_migration(tmp_path) -> None:
    """o5-D13: возрастная обрезка и архивация фильтруют по received_at без индекса.
    Индекс нельзя класть в _SCHEMA: на старой БД колонки ещё нет и executescript
    упадёт — только после _migrate_add_columns."""
    import sqlite3

    from server import db

    path = tmp_path / "legacy.db"
    conn = sqlite3.connect(path)  # БД БЕЗ received_at — как до миграции
    conn.executescript(
        "CREATE TABLE heartbeats (id INTEGER PRIMARY KEY, device_id TEXT, ts TEXT, payload TEXT);"
        "CREATE TABLE events (id INTEGER PRIMARY KEY, device_id TEXT, ts TEXT, event_id INTEGER);"
    )
    conn.commit()
    conn.close()
    ...
    assert "idx_hb_received" in names
    assert "idx_ev_received" in names
