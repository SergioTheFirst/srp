...
def test_cert_script_guards_each_certificate() -> None:
    """o5-C14: битый сертификат в хранилище не должен обнулять весь список."""
    from client.collectors.historical import _CERT_SCRIPT

    assert "} catch { continue }" in _CERT_SCRIPT
    head = _CERT_SCRIPT.split("foreach ($c in $items)")[0]
    assert not head.rstrip().endswith("try {"), "внешний try вокруг цикла сертификатов остался"
    # Перечисление хранилища при этом обязано остаться под собственной защитой.
    assert "try { $items = @(Get-ChildItem" in _CERT_SCRIPT
