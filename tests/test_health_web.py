...
def _record_sql(monkeypatch):
    """Перехватить SQL, который функция реально выполняет (не переписанный в тесте)."""
    from server import db
...
def test_fleet_health_reads_only_newest_score_row(client, monkeypatch) -> None:
    """o5-D4: ROW_NUMBER() заставляет SQLite прочитать и отсортировать ВСЮ таблицу
    scores ради одной строки на устройство -- на парке с историей это доминирующая
    стоимость страницы здоровья. Коррелированный максимум работает по
    idx_scores_device (та же форма, что уже в get_devices)."""
    from server import db

    risk = '{"health": {"state": "h0", "index": 90, "band": "good"}}'
    with db._connect() as conn:
        for dev in ("fh-1", "fh-2", "fh-3"):
            conn.execute(
                "INSERT OR IGNORE INTO devices (device_id, hostname) VALUES (?,?)", (dev, dev)
            )
            conn.executemany(
                "INSERT INTO scores (device_id, ts, risk) VALUES (?,?,?)",
                [(dev, f"2026-03-01T10:{i:02d}:00+00:00", risk) for i in range(300)],
            )
        conn.commit()

    seen = _record_sql(monkeypatch)
    rows = db.get_fleet_health()
    assert len(rows) == 3  # ровно последний скор каждого устройства

    sql = next(q for q in seen if "health.state" in q)
    assert "ROW_NUMBER" not in sql.upper(), "полный скан scores вместо коррелированного максимума"
    with db._connect() as conn:
        plan = " ".join(str(r[3]) for r in conn.execute("EXPLAIN QUERY PLAN " + sql))
    assert "idx_scores_device" in plan


def test_fleet_health_deltas_reads_only_capped_tail(client, monkeypatch) -> None:
    """o5-D4: та же оконная функция в get_fleet_health_deltas."""
    from server import db

    seen = _record_sql(monkeypatch)
    db.get_fleet_health_deltas()
    sql = next(q for q in seen if "health.state" in q)
