...
def _wait_until(predicate, timeout: float = 2.0, interval: float = 0.005) -> bool:
    ...
    o5-B2: a stale/invalidated cache read is now served at once, with the reload
    running on a background thread -- tests that assert on the reloaded graph poll
    for it instead of asserting on the very next read.
    """
...
def test_network_map_graph_reloads_after_ttl_lapses(client: TestClient) -> None:
    ...
    # o5-B2: past the TTL, get() serves the last graph at once and rebuilds it on a
    # background thread -- poll for the rebuilt graph instead of asserting on the
    # very next read.
    ...
    # advance past the TTL -> reads start triggering a background rebuild
    now["v"] += 6.0

    def _has_new_node() -> bool:
        return "nd-mac-DEADBEEF0003" in _nodes(client.get("/api/v1/network-map/graph").json())

    assert _wait_until(_has_new_node)
...
def test_discovery_poll_invalidates_network_map_cache(client: TestClient) -> None:
    ...
    # discovery cycle: the cache must drop so the next read shows the new node.
    # o5-B2: invalidate() no longer clears the cached value -- the next read serves
    # the stale graph at once and rebuilds in the background, so poll for the new
    # node instead of asserting on the very next read.
    ...
    def _has_node() -> bool:
        return "nd-mac-DEADBEEF0099" in _nodes(client.get("/api/v1/network-map/graph").json())

    assert _wait_until(_has_node)


def test_topology_poll_invalidates_network_map_cache(client: TestClient) -> None:
    # o5-B2: the force-poll now returns before its own cycle finishes, and the
    # cache invalidation it triggers is itself served-stale-then-rebuilt -- poll
    # for the new node instead of asserting on the very next read.
    assert client.get("/api/v1/network-map/graph").json()["totals"]["nodes"] == 0
    db.upsert_net_device(
    ...
    def _has_node() -> bool:
        return "nd-mac-DEADBEEF0077" in _nodes(client.get("/api/v1/network-map/graph").json())
...
def test_printer_poll_invalidates_network_map_cache(
    ...
    # o5-B2: invalidate() no longer clears the cached value -- poll for the new node
    # instead of asserting on the very next read.
    assert client.get("/api/v1/network-map/graph").json()["totals"]["nodes"] == 0
    db.upsert_net_device(
    ...
    def _has_node() -> bool:
        return "nd-mac-DEADBEEF0066" in _nodes(client.get("/api/v1/network-map/graph").json())
