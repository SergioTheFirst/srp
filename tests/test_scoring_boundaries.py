...
def test_risk_exposure_ignores_whea_noise() -> None:
    """o5-E2: исправленные ошибки WHEA — преимущественно шум прошивки/ASPM/PCIe.
    Как прямой драйвер риска WHEA убран решением W4.3/D6 (см. bayesian.py), но в
    risk_exposure он оставался — тот же ложняк на другой оси."""
    from server.scoring.scores import _risk_exposure

    value, factors = _risk_exposure(None, {"whea_errors_30d": 15}, None)
    assert value == 0.0
    assert not any("WHEA" in f["label"] for f in factors)
