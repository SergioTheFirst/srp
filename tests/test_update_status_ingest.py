...
def test_ingest_update_status_stores_available_version(client) -> None:
    r = client.post(
        "/api/v1/ingest",
        json=envelope(
            "dev-upd-avail",
            "update_status",
            {"state": "ok", "available_version": "0.3.0"},
        ),
    )
    assert r.status_code == 200, r.text
    d = db.get_device("dev-upd-avail")
    assert d["update_available_version"] == "0.3.0"


def test_ingest_update_status_resets_available_version_when_absent(client) -> None:
    """D3: агент перестал видеть pending-версию (обновился) -> старое значение
    должно очиститься, иначе «ждёт 0.3.0» повиснет навсегда."""
    device_id = "dev-upd-clear"
    client.post(
        "/api/v1/ingest",
        json=envelope(device_id, "update_status", {"state": "ok", "available_version": "0.3.0"}),
    )
    assert db.get_device(device_id)["update_available_version"] == "0.3.0"

    client.post(
        "/api/v1/ingest",
        json=envelope(device_id, "update_status", {"state": "ok"}),
    )
    assert db.get_device(device_id)["update_available_version"] is None
...
# --------------------------------------------------------------------------- #
# U2-b: a replayed OLDER update_status envelope must not overwrite a newer
# stored one. client/transport.py's flush_buffer can resend a stale
# update_status after a fresher one already landed; update_checked_at already
# holds the CLIENT-reported checked_at (see client/updater.py), so it is
# directly comparable to the incoming payload's checked_at.
# --------------------------------------------------------------------------- #
def test_set_update_status_newer_then_older_keeps_newer(client) -> None:
    device_id = "dev-upd-replay"
    client.post(
        "/api/v1/ingest",
        json=envelope(
            device_id,
            "update_status",
            {"state": "ok", "checked_at": "2026-08-01T12:00:00+00:00"},
        ),
    )
    assert db.get_device(device_id)["update_checked_at"] == "2026-08-01T12:00:00+00:00"

    # Buffered replay of an OLDER update_status (e.g. a "failed" attempt from
    # before the agent finally succeeded, replayed after the fact).
    client.post(
        "/api/v1/ingest",
        json=envelope(
            device_id,
            "update_status",
            {"state": "failed", "error": "old attempt", "checked_at": "2026-08-01T10:00:00+00:00"},
        ),
    )
    d = db.get_device(device_id)
    assert d["update_state"] == "ok"
    assert d["update_error"] is None
    assert d["update_checked_at"] == "2026-08-01T12:00:00+00:00"


def test_set_update_status_older_then_newer_applies_newer(client) -> None:
    device_id = "dev-upd-order"
    client.post(
        "/api/v1/ingest",
        json=envelope(
            device_id,
            "update_status",
            {"state": "failed", "error": "attempt 1", "checked_at": "2026-08-01T10:00:00+00:00"},
        ),
    )
    client.post(
        "/api/v1/ingest",
...
def test_set_update_status_direct_call_skips_write_on_older_checked_at(client) -> None:
    """Direct db.set_update_status unit check (bypasses touch_device/pipeline)."""
    device_id = "dev-upd-direct"
    db.touch_device(device_id, "2026-08-01T12:00:00+00:00", "0.2.0")
    db.set_update_status(device_id, "ok", None, "2026-08-01T12:00:00+00:00")
    db.set_update_status(device_id, "failed", "stale", "2026-08-01T09:00:00+00:00")
    d = db.get_device(device_id)
    assert d["update_state"] == "ok"
    assert d["update_error"] is None
    assert d["update_checked_at"] == "2026-08-01T12:00:00+00:00"
...
def test_set_update_status_future_stored_checked_at_is_not_a_permanent_ceiling(client) -> None:
    """F5/MEDIUM-1: a stored checked_at far in the future (agent clock skewed
    forward, or a forged envelope) must not gate every later real status
    forever -- only a stored value within _UPDATE_STATUS_FUTURE_TOLERANCE of
    the SERVER clock still acts as a replay guard. A normal replay (older
    than a recent, non-future stored value) is still covered by
    test_set_update_status_direct_call_skips_write_on_older_checked_at above."""
    device_id = "dev-upd-future"
    db.touch_device(device_id, "2026-08-01T12:00:00+00:00", "0.2.0")
    future = (datetime.now(timezone.utc) + timedelta(days=3)).isoformat()
    db.set_update_status(device_id, "ok", None, future)
    assert db.get_device(device_id)["update_checked_at"] == future

    now = datetime.now(timezone.utc).isoformat()
    db.set_update_status(device_id, "failed", "настоящая ошибка", now)
    d = db.get_device(device_id)
    assert d["update_state"] == "failed"
    assert d["update_error"] == "настоящая ошибка"
    assert d["update_checked_at"] == now


def test_set_update_status_unparseable_checked_at_still_applies(client) -> None:
    """No comparable timestamp on one side -> can't prove a replay -> apply
    (unchanged pre-U2-b behaviour)."""
    device_id = "dev-upd-noparse"
    db.touch_device(device_id, "2026-08-01T12:00:00+00:00", "0.2.0")
    db.set_update_status(device_id, "ok", None, "2026-08-01T12:00:00+00:00")
    db.set_update_status(device_id, "updating", None, None)
    d = db.get_device(device_id)
    assert d["update_state"] == "updating"
    assert d["update_checked_at"] is None
