...
def test_fleet_page_carries_admin_token_meta(client) -> None:
    """o5-D2: кнопки удаления/очистки шлют токен из мета-тега — без него они
    молча получают 401 при включённой аутентификации."""
    html = client.get("/").text
    assert 'name="srp-token"' in html
    assert "x-srp-token" in html
