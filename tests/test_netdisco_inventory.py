...
def test_neighbor_last_seen_uses_snapshot_time_not_agent_heartbeat() -> None:
    # D2: a neighbour's last_seen must reflect when the ARP snapshot was taken,
    # never the agent's own (possibly much newer) heartbeat -- an agent can keep
    # heartbeating for hours after this particular capture went stale.
    snap = {
        **_SNAP_A,
        "last_seen": "2026-06-20T18:00:00+00:00",  # agent heartbeat -- much newer
        "snapshot_at": "2026-06-20T10:05:00+00:00",  # when this ARP data was captured
    }
    by = {d.nid: d for d in build_inventory([snap])}
    gw = by["nd-mac-00-50-56-AA-BB-CC"]
    assert gw.last_seen == "2026-06-20T10:05:00+00:00"


def test_neighbor_last_seen_falls_back_to_agent_last_seen_without_snapshot_at() -> None:
    # Old net_snapshots rows (or hand-built fixtures) without the snapshot_at
    # column must keep the pre-D2 behaviour, not crash or go None.
    by = {d.nid: d for d in build_inventory([_SNAP_A])}
    gw = by["nd-mac-00-50-56-AA-BB-CC"]
    assert gw.last_seen == _SNAP_A["last_seen"]
...
_SNAP_NETBIOS: dict[str, Any] = {
    ...
    "neighbors": [
        {"ip": "10.0.0.7", "mac": "22-33-44-55-66-77", "name": "DEMO2", "name_source": "netbios"}
    ],
}
...
def test_neighbor_netbios_name_seeds_hostname_hint() -> None:
    ...
    assert neighbor.hostname_hint == "DEMO2"
...
def test_hostile_netbios_name_is_dropped_at_the_boundary() -> None:
    ...
    devices = build_inventory([_snap("DEMO2")])  # a clean name still passes
    neighbor = next(d for d in devices if d.nid == "nd-mac-22-33-44-55-66-77")
    assert neighbor.hostname_hint == "DEMO2"
...
def test_persist_inventory_fills_hostname_from_netbios_hint_when_empty() -> None:
    ...
    assert filled["nd-mac-22-33-44-55-66-77"]["hostname"] == "DEMO2"
