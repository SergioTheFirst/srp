...
def test_connection_busy_timeout_matches_connect_timeout(tmp_path) -> None:
    """o5-D15: PRAGMA busy_timeout=5000 ПОНИЖАЛА уже заданный sqlite3.connect
    (timeout=10.0) — под нагрузкой писатель сдавался вдвое раньше положенного."""
    from server import db

    db.init_db(str(tmp_path / "bt.db"))
    with db._connect() as conn:
        assert conn.execute("PRAGMA busy_timeout").fetchone()[0] == 10000


def test_wal_pragma_applied_once_per_db(tmp_path, monkeypatch) -> None:
    """o5-D15: journal_mode=WAL — свойство ФАЙЛА, а не соединения; выставлять его
    на каждом подключении означает лишний служебный запрос на каждый запрос."""
    import sqlite3
    ...
    monkeypatch.setattr(
        db.sqlite3, "connect", lambda *a, **kw: real_connect(*a, factory=_SpyConn, **kw)
    )
    for _ in range(5):
        with db._connect() as conn:
            conn.execute("SELECT 1")

    wal = [q for q in seen if "journal_mode" in q]
    assert len(wal) <= 1, f"WAL выставляется на каждом соединении: {len(wal)} раз"
