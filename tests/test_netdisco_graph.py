...
def test_chokepoints_linear_chain_1000_nodes_under_one_second():
    # a 1000-node chain n0-n1-...-n999: every edge is a bridge (no cycles at all).
    # Proves the O(V+E) Tarjan pass, not the O(V*(V+E)) bruteforce component recount.
    nodes = [f"n{i}" for i in range(1000)]
    devices = [{"device_nid": n} for n in nodes]
    links = [{"a_nid": nodes[i], "b_nid": nodes[i + 1]} for i in range(999)]
    graph = g.build_graph(devices, links)
    start = time.monotonic()
    bridges = g.find_bridges(graph)
    elapsed = time.monotonic() - start
    assert len(bridges) == 999
    assert elapsed < 1.0


def test_tarjan_matches_bruteforce_on_fixtures():
    # Regression anchor: the Tarjan rewrite must reproduce exactly the same sets the
    # brute-force component-recount implementation produced on these fixtures.
    tree = _graph()
    assert g.find_articulation_points(tree) == {"gw", "sw"}
    assert g.find_bridges(tree) == {
        frozenset({"R", "gw"}),
        frozenset({"gw", "h1"}),
        frozenset({"gw", "h2"}),
        frozenset({"gw", "sw"}),
    ...
    cycle = g.build_graph(_CYCLE_DEVICES, _CYCLE_LINKS)
    assert g.find_articulation_points(cycle) == {"c"}
    assert g.find_bridges(cycle) == {frozenset({"c", "d"})}
