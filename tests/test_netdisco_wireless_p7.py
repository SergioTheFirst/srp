...
def test_collect_wireless_non_string_sys_object_id_is_fail_closed():
    # F1: sys_object_id can arrive as a live SNMP GET from an untrusted device;
    # the decoder may hand back an int (INTEGER tag) instead of str. Must not
    # raise (AttributeError on .startswith) and must not walk anything.
    session = FakeSession(
        {oids.BSN_MOBILE_STATION_AP_MAC: {f"{oids.BSN_MOBILE_STATION_AP_MAC}.1": "x"}}
    )
    assert wireless.collect_wireless(session, sys_object_id=12345) == []  # type: ignore[arg-type]
