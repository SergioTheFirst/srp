...
def test_is_rfc1918_accepts_all_three_private_blocks():
    ...
    assert lan_scan._is_rfc1918("192.168.77.6")
...
def test_own_lan_cidrs_derives_slash24_for_rfc1918():
    assert lan_scan.own_lan_cidrs(["192.168.77.6"]) == ["192.168.77.0/24"]
...
def test_own_lan_cidrs_dedupes_same_segment():
    assert lan_scan.own_lan_cidrs(["192.168.77.6", "192.168.77.25"]) == ["192.168.77.0/24"]


def test_own_lan_cidrs_sorts_multiple_segments():
    assert lan_scan.own_lan_cidrs(["192.168.77.6", "10.0.0.5"]) == [
        "10.0.0.0/24",
        "192.168.77.0/24",
    ]
...
def test_expand_hosts_enumerates_full_slash24():
    hosts = lan_scan.expand_hosts(["192.168.77.0/24"])
    assert len(hosts) == 254  # .1 - .254; network/broadcast excluded
    assert "192.168.77.1" in hosts
    assert "192.168.77.254" in hosts
    assert "192.168.77.0" not in hosts
    assert "192.168.77.255" not in hosts


def test_expand_hosts_respects_max_hosts_cap():
    assert len(lan_scan.expand_hosts(["192.168.77.0/24"], max_hosts=10)) == 10


def test_expand_hosts_dedupes_overlapping_cidrs():
    assert len(lan_scan.expand_hosts(["192.168.77.0/24", "192.168.77.0/24"])) == 254
...
def test_expand_hosts_max_hosts_zero_is_kill_switch():
    assert lan_scan.expand_hosts(["192.168.77.0/24"], max_hosts=0) == []
...
def test_sweep_host_returns_true_on_first_open_port_and_stops():
    ...
    assert lan_scan.sweep_host("192.168.77.6", touch=spy) is True
    # stopped after 443 answered -- 445/3389/9100 never tried
    assert spy.calls == [("192.168.77.6", 80), ("192.168.77.6", 443)]


def test_sweep_host_all_closed_returns_false_and_tries_every_port():
    spy = _TouchSpy({})  # every port defaults False
    assert lan_scan.sweep_host("192.168.77.6", touch=spy) is False
...
def test_sweep_host_one_port_raising_does_not_abort_the_host():
    ...
    assert lan_scan.sweep_host("192.168.77.6", touch=spy) is True
    assert ("192.168.77.6", 443) in spy.calls  # continued past the raising port
...
def test_sweep_checks_only_hosts_inside_the_derived_cidr():
    ...
    lan_scan.sweep(["192.168.77.6"], host_check=host_check)
    assert seen  # something was checked
    assert all(ip.startswith("192.168.77.") for ip in seen)
    assert "192.168.77.6" in seen


def test_sweep_returns_count_of_answering_hosts():
    def host_check(ip):
        return ip in ("192.168.77.10", "192.168.77.20")

    assert lan_scan.sweep(["192.168.77.6"], host_check=host_check) == 2
...
def test_sweep_respects_max_hosts():
    ...
    lan_scan.sweep(["192.168.77.6"], max_hosts=5, host_check=host_check)
    assert len(seen) == 5
...
def test_sweep_max_hosts_zero_kill_switch_returns_zero():
    ...
    assert lan_scan.sweep(["192.168.77.6"], max_hosts=0, host_check=boom) == 0
