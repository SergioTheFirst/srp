...
against real Windows hosts on a real LAN), so this module now shells out to
...
NetBIOS Remote Machine Name Table
   ...
   DEMOPC1        <00>  UNIQUE      Registered
   DEMOPC1        <20>  UNIQUE      Registered
...
def test_parse_extracts_name_from_suffix_0x20_line():
    assert lan_names._parse_nbtstat(_REAL_OUTPUT) == "DEMOPC1"
...
def test_parse_is_locale_independent_of_status_words():
    ...
    localized = (
        "\n   DEMOPC1        <00>  УНИКАЛЬНЫЙ   Зарегистрирован\n"
        "   DEMOPC1        <20>  УНИКАЛЬНЫЙ   Зарегистрирован\n"
        "   WORKGROUP      <00>  ГРУППА       Зарегистрирован\n"
    )
    assert lan_names._parse_nbtstat(localized) == "DEMOPC1"

    garbled = "   DEMOPC1        <20>  ���      ���\n"
    assert lan_names._parse_nbtstat(garbled) == "DEMOPC1"


def test_parse_fail_closed_without_suffix_0x20():
    text = (
        "   DEMOPC1        <00>  UNIQUE      Registered\n"
        "   DEMOPC1        <1C>  GROUP       Registered\n"
    )
...
def test_resolve_returns_name_for_responder():
    out = lan_names.resolve_netbios_names(
        ["10.0.0.3"], runner=_canned({"10.0.0.3": _nbtstat_text("DEMOPC1")})
    )
    assert out == {"10.0.0.3": "DEMOPC1"}
...
def test_resolve_multiple_responders():
    table = {
        "192.168.77.6": _nbtstat_text("DEMOPC1"),
        "192.168.77.25": _nbtstat_text("DEMO2"),
    }
    out = lan_names.resolve_netbios_names(["192.168.77.6", "192.168.77.25"], runner=_canned(table))
    assert out == {"192.168.77.6": "DEMOPC1", "192.168.77.25": "DEMO2"}
