...
def test_wireless_client_yields_wireless_link_to_ap() -> None:
    # A9/A8: the controller knows a client is on radio (is_wired=False) with a
    # concrete ap_mac -- that must become a wireless AdapterLink, not silence.
    def transport(path: str) -> Dict[str, Any]:
        if path.endswith("/stat/sta"):
            return _envelope(
                [{"mac": "aa:bb:cc:00:03:01", "is_wired": False, "ap_mac": "bb:cc:dd:00:00:01"}]
            )
        return _envelope([])
    ...
    link = out.links[0]
    assert normalize_mac(link.a_mac) == normalize_mac("aa:bb:cc:00:03:01")
    assert normalize_mac(link.b_mac) == normalize_mac("bb:cc:dd:00:00:01")
    assert link.link_kind == "wireless"
