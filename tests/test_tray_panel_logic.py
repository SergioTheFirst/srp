...
def test_parse_args_edit_owner() -> None:
    """F11: новый флаг режима «Мои данные» (по образцу --stop-agent)."""
    from client.tray.__main__ import _parse_args

    assert _parse_args(["--edit-owner"]).edit_owner is True
    assert _parse_args([]).edit_owner is False


def test_main_dispatches_edit_owner(monkeypatch, tmp_path: Path) -> None:
    from client.tray import __main__ as tm

    called: dict = {}

    def fake_run_owner_form(*, config_path: Path) -> int:
        called["config_path"] = config_path
        return 0

    monkeypatch.setattr(tm.panel, "run_owner_form", fake_run_owner_form)
    monkeypatch.setattr(tm, "_config_path", lambda: tmp_path / "config.json")

    assert tm.main(["--edit-owner"]) == 0
    assert called["config_path"] == tmp_path / "config.json"
