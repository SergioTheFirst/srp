...
def test_gate_axis_not_applicable_required_domain_withholds() -> None:
    """o5-E4: NOT_APPLICABLE обязательного домена — это «датчика нет на этом
    оборудовании», а не «всё хорошо». Гейт ловил только unknown, поэтому ось
    выдавала уверенное значение по отсутствующему источнику."""
    from server.scoring.score100 import _gate_axis

    trust = {"domains": {"storage": {"state": "not_applicable"}}}
    s = _gate_axis(
        numeric=42.0,
    ...
    assert s.confidence != "high", "NOT_APPLICABLE выдан за доверие"
