...
from shared.schema import CONTRACT_VERSION, HistoricalPayload, NetAdapter
...
def test_network_fields_round_trip():
    ...
    assert p.network_routes[0].next_hop == "10.0.77.1"
...
def test_neighbor_name_and_name_source_round_trip():
    ...
    assert p.network_neighbors[0].name == "DEMOPC1"
...
def test_route_if_index_and_metric_absent_is_fine():
    ...
    p = HistoricalPayload(network_routes=[{"dest": "10.20.0.0/16", "next_hop": "10.0.77.1"}])
...
# --------------------------------------------------------------------------- #
# A5: NDIS physical medium on NetAdapter (additive; no CONTRACT_VERSION bump) #
# --------------------------------------------------------------------------- #


def test_net_adapter_accepts_phys_medium():
    a = NetAdapter(phys_medium=9)
    assert a.phys_medium == 9
    b = NetAdapter()
    assert b.phys_medium is None
    assert CONTRACT_VERSION == "0.1.0"
