...
def test_recompute_reads_events_once(client, monkeypatch) -> None:
    """o5-D16: пересчёт дважды тянет один и тот же журнал событий (1000 строк для
    цепочки отказов и 200 для трендов) — второй запрос строго избыточен, это
    префикс первого."""
    from server import db, pipeline

    from tests.conftest import envelope, healthy

    client.post("/api/v1/ingest", json=envelope("d16-dev", "historical", healthy("historical")))

    calls = {"n": 0}
    real = db.get_recent_events

    def _counting(device_id, limit=20, **kw):
        calls["n"] += 1
        return real(device_id, limit=limit, **kw)

    monkeypatch.setattr(db, "get_recent_events", _counting)
    pipeline.recompute_scores("d16-dev")
    assert calls["n"] == 1, f"журнал событий прочитан {calls['n']} раз(а) за пересчёт"
