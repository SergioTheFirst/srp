...
def test_wireless_medium_from_wifi_uplink_and_ap_link():
    ...
    # o5-A4 contract change: the old Ф2 heuristic painted ANY link touching an AP node
    # "wireless", including a switch<->AP backbone uplink discovered via LLDP -- that
    # is itself a wired cable, not a client association, so the old assertion pinned a
    # false positive. The new heuristic only calls "wireless" an AP link whose OTHER
    # end is a client type (endpoint/unknown/phone); a switch neighbour on an
    # LLDP-sourced link now correctly resolves to "wired".
    sw, ap = device_nid(mac=SW), device_nid(mac=AP)
    g2 = build_network_map(
        [_nd(sw, "switch", mac=SW), _nd(ap, "ap", mac=AP)], [_link(sw, ap)], [], []
    )
    assert g2["links"][0]["medium"] == "wired"


def test_wireless_medium_for_ap_to_client_link():
    # The corrected o5-A4 counterpart of the case above: an AP link to an actual
    # client endpoint (not another infra device) still resolves to "wireless".
    ap, ep = device_nid(mac=AP), device_nid(mac=A1)
    g = build_network_map(
        [_nd(ap, "ap", mac=AP), _nd(ep, "endpoint", mac=A1)], [_link(ap, ep)], [], []
    )
    assert g["links"][0]["medium"] == "wireless"
...
# --- o5-A11: an agent must dedup by ANY adapter, not only the primary one ---
def test_merge_agents_dedups_by_any_adapter_not_just_primary():
    # net_devices already knows the SECOND adapter's MAC (A2); the snapshot's
    # adapters[0] carries a DIFFERENT mac (A1). Before A11 _merge_agents only
    # checked the primary adapter against by_mac/by_ip, so this synthesized a
    # SECOND node for device_nid(mac=A1) instead of merging into the net_device
    # already keyed on A2 -- the agent doubled on the map.
    nid2 = device_nid(mac=A2)
    net_devices = [_nd(nid2, "unknown", mac=A2)]
    snap = {
        "device_id": "d1",
        "hostname": "pc-d1",
        "adapters": [
    ...
    g = build_network_map(net_devices, [], [snap], [])
    assert len(g["nodes"]) == 1
    n = g["nodes"][0]
    assert n["nid"] == nid2
    assert n["device_id"] == "d1"


# --- F5: _agent_nid must not match on a virtual/tunnel adapter's MAC or IP ---
def _snap_with_virtual_adapter(did, mac, ip):
    return {
        "device_id": did,
        "hostname": f"pc-{did}",
        "adapters": [
...
def test_agent_nid_ignores_virtual_adapter_shared_across_fleet():
    # F5: a fleet-wide address like VirtualBox's 192.168.56.1 host-only IP is
    # NOT unique to one machine. If some other already-known net_device happens
    # to carry it (e.g. from a different host's own passive discovery), scanning
    # a virtual adapter's IP against by_ip must not steal that unrelated node's
    # identity for two different real agents -- worse than the original doubling
    # this was meant to fix.
    phantom_nid = "nd-mac-DE-AD-00-00-00-09"
    net_devices = [_nd(phantom_nid, ip="192.168.56.1", mac="DE-AD-00-00-00-09")]
    snap1 = _snap_with_virtual_adapter("d1", "AA-BB-CC-00-00-01", "10.0.0.11")
    snap2 = _snap_with_virtual_adapter("d2", "AA-BB-CC-00-00-02", "10.0.0.12")
    g = build_network_map(net_devices, [], [snap1, snap2], [])
    agent_nids = {n["nid"] for n in g["nodes"] if n.get("device_id")}
    assert agent_nids == {device_nid(mac="AA-BB-CC-00-00-01"), device_nid(mac="AA-BB-CC-00-00-02")}
