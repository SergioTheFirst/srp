...
def test_replace_net_links_medium_is_latest_wins_not_coalesced(tmp_path: Path) -> None:
    # F6: fusion recomputes medium from ALL evidence every cycle -- an old
    # fail-open "wired" row must be overwritable by an honest NULL, or A4's goal
    # (UNKNOWN over false confidence) never reaches an existing installation.
    p = tmp_path / "srp.db"
    db.init_db(p)
    base = {"a_nid": "nd-a", "b_nid": "nd-z", "link_kind": "l2-edge", "via_source": "fdb_edge"}
    db.replace_net_links([{**base, "confidence": "high", "medium": "wired"}], {"nd-a"})
    db.replace_net_links([{**base, "confidence": "high", "medium": None}], {"nd-a"})
    row = _rows(p, "SELECT medium FROM net_links")[0]
    assert row["medium"] is None

    # upsert_net_link (adapter/enrichment path) keeps its COALESCE -- adapters only
    # enrich, never overwrite an SNMP-derived medium.
    db.upsert_net_link({**base, "confidence": "high", "medium": "wireless"})
    db.upsert_net_link({**base, "confidence": "high", "medium": None})
    row = _rows(p, "SELECT medium FROM net_links")[0]
