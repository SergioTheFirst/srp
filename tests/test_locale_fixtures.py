...
# --------------------------------------------------------------------------- #
# PS 5.1 floor: no PowerShell 6+ only parameters anywhere in the agent          #
# --------------------------------------------------------------------------- #


def _client_ps_scripts() -> dict[str, str]:
    """Все PowerShell-константы пакета client/ (обходом модулей, без чтения файлов)."""
    import importlib
    import pkgutil
    ...
    for mod_info in pkgutil.walk_packages(client.__path__, prefix="client."):
        try:
            mod = importlib.import_module(mod_info.name)
        except Exception:  # noqa: BLE001 -- модуль без Windows-зависимостей нам не нужен
            continue
        for name, value in vars(mod).items():
            if isinstance(value, str) and ("Get-" in value or "$" in value and "\n" in value):
                scripts[f"{mod_info.name}.{name}"] = value
    return scripts


def test_no_ps6_only_params_in_client_scripts():
    """o5-C4: -TimeoutSeconds не существует у Test-Connection в Windows PowerShell 5.1
    (пол платформы) -- скрипт с ним молча теряет весь результат на проде."""
    offenders = [n for n, s in _client_ps_scripts().items() if "-TimeoutSeconds" in s]
    assert offenders == [], f"PS6-only параметр в скриптах агента: {offenders}"
