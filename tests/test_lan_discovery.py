...
def test_is_rfc1918_accepts_all_three_private_blocks():
    ...
    assert lan_discovery._is_rfc1918("192.168.77.6")
...
def test_open_listener_joins_each_member_ip():
    ...
    out = lan_discovery._open_listener(
        "224.0.0.251", 5353, ["192.168.77.5", "192.168.77.6"], socket_factory=lambda *a: sock
    )
    assert out is sock
...
def test_open_listener_returns_none_when_bind_fails():
    ...
    out = lan_discovery._open_listener(
        "224.0.0.251", 5353, ["192.168.77.5"], socket_factory=lambda *a: sock
    )
    assert out is None
    assert sock.closed
...
def test_open_listener_returns_none_when_socket_factory_raises():
    ...
    out = lan_discovery._open_listener(
        "224.0.0.251", 5353, ["192.168.77.5"], socket_factory=factory
    )
    assert out is None


def test_open_listener_one_bad_adapter_does_not_sink_the_others():
    sock = FakeSocket(join_fails=("192.168.77.5",))
    out = lan_discovery._open_listener(
        "224.0.0.251",
        5353,
        ["192.168.77.5", "192.168.77.6"],
...
def test_open_listener_returns_none_when_all_joins_fail():
    sock = FakeSocket(join_fails=("192.168.77.5", "192.168.77.6"))
    out = lan_discovery._open_listener(
        "224.0.0.251",
        5353,
        ["192.168.77.5", "192.168.77.6"],
...
def test_capture_dedupes_by_source_and_ip_keeping_first():
    sock = FakeSocket([(b"first", ("192.168.77.5", 0)), (b"second", ("192.168.77.5", 0))])
...
def test_capture_enforces_cap():
    queue = [(f"p{i}".encode(), (f"192.168.77.{i}", 0)) for i in range(5)]
...
def test_capture_truncates_raw_payload_to_max_hint_bytes():
    ...
    sock = FakeSocket([(big, ("192.168.77.5", 0))])
    out = lan_discovery._capture(
        {sock: "mdns"}, deadline=10.0, cap=128, now_fn=_FakeClock(), sleep_fn=lambda s: None
    )
...
def test_capture_tags_each_hint_with_its_own_socket_source():
    mdns_sock = FakeSocket([(b"m", ("192.168.77.6", 0))])
    ssdp_sock = FakeSocket([(b"s", ("192.168.77.7", 0))])
    out = lan_discovery._capture(
    ...
    assert by_source["mdns"]["ip"] == "192.168.77.6"
    assert by_source["ssdp"]["ip"] == "192.168.77.7"
...
def test_collect_lan_discovery_happy_path_relays_from_all_protocols():
    mdns_sock = FakeSocket([(b"mdns-payload", ("192.168.77.6", 5353))])
    ssdp_sock = FakeSocket([(b"ssdp-payload", ("192.168.77.7", 1900))])
...
def test_collect_lan_discovery_all_listeners_fail_returns_empty():
    ...
    out = lan_discovery.collect_lan_discovery(["192.168.77.5"], socket_factory=factory)
    assert out == []


def test_collect_lan_discovery_caps_total_hints():
    queue = [(f"p{i}".encode(), (f"192.168.77.{i}", 0)) for i in range(10)]
