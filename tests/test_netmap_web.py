...
def test_netmap_poll_button_targets_the_full_collect_pipeline(client):
    """D1: the button now drives the full server-side collect job, not the old
    single-cycle topology-only poll."""
    _ingest(client, "map-44", _net_payload())
    body = client.get("/netmap").text
    assert "/api/v1/network-map/collect" in body
    assert "/api/v1/topology/poll" not in body


def test_netmap_poll_button_handles_skipped_and_cooldown_responses(client):
    """F8: the JS must handle the two new POST response shapes -- netdisco off
    (``skipped``) and the 5-minute cooldown (``cooldown``) -- distinctly from
    the pre-existing ``busy`` case, re-enabling the button without polling
    status or reloading in either case."""
    body = client.get("/netmap").text
    assert "обнаружение сети выключено в конфигурации сервера" in body
    assert "карта собиралась менее 5 минут назад" in body
    assert "d.retry_after_sec" in body
...
def test_netgraph_has_unknown_medium_filter(client):
    """o5-A4: medium can be genuinely unknown (no fail-open to "wired") -- the filter
    panel's MED array must include it so operators can see/filter these edges."""
    _ingest(client, "map-medium-unknown", _net_payload())
    body = client.get("/netmap").text
    m = re.search(r"var MED = (\[[^\]]*\]);", body)
    assert m, "MED array declaration missing"
    assert "unknown" in m.group(1)


def test_netgraph_draws_discovered_status(client):
    """Канвас знал только 4 статуса и ПРЯЧЕТ узел с любым другим: шлюз из
    активного обнаружения приходит со status="discovered" (scheduler.py:258,
    adapter_merge.py:80) → узел-роутер не рисовался, а все аплинки агент→шлюз
    уходили в dim. Пины: discovered в фильтре STATUS, русская подпись в statusRu,
    и fail-open в nodeOk — статус вне фильтра узел НЕ прячет."""
    _ingest(client, "map-disc", _net_payload())
    body = client.get("/netmap").text
    m = re.search(r"var STATUS = (\[[^\]]*\]);", body)
    assert m, "STATUS array declaration missing"
    assert "discovered" in m.group(1)
    assert "обнаружен" in body  # statusRu: тултип/панель/чекбокс фильтра
    assert "n.status in state.status" in body, "nodeOk прячет неизвестные статусы (fail-closed)"


def test_net_status_ru_knows_discovered():
    """Таблица инвентаря сети: discovered → («na», «обнаружен»), не «неизвестно»."""
    from server.web.dashboard import net_status_ru

    assert net_status_ru("discovered") == ("na", "обнаружен")
    assert net_status_ru("bogus") == ("na", "неизвестно")  # прежний fallback не сломан
