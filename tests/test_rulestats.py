...
def test_rulestats_scan_uses_one_connection(tmp_path, monkeypatch) -> None:
    """o5-D14: скан открывал новое соединение на КАЖДОЕ устройство (через
    get_score_series) — на парке это сотни открытий за проход обслуживания."""
    from server import db

    db.init_db(str(tmp_path / "rs.db"))
    with db._connect() as conn:
        for i in range(20):
            conn.execute(
                "INSERT INTO devices (device_id, hostname) VALUES (?,?)", (f"r{i}", f"r{i}")
            )
        conn.commit()
    ...
    monkeypatch.setattr(db, "_connect", _counting)
    db.run_rulestats_scan()
    assert calls["n"] <= 3, f"открыто {calls['n']} соединений на 20 устройств"
