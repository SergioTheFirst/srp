...
def test_purge_401_has_no_count_key(client: TestClient) -> None:
    """o5-D2: при настроенном токене неавторизованный purge обязан отдать 401 и НЕ
    класть в тело `count` — дашборд читал `p.count || 0` и рисовал «нет машин»
    вместо честного «отказано»."""
    client.app.state.ingest_token = "t"
    try:
        r = client.post("/api/v1/devices/purge", json={"days": 30, "dry_run": True})
        assert r.status_code == 401
        assert "count" not in r.json()
    finally:
        client.app.state.ingest_token = ""


def test_dashboard_never_publishes_ingest_token(client) -> None:
    """Ревью блока D (H4): мета-тег с ingest-токеном обнулял бы единственный
    механизм аутентификации ровно в момент его включения — снять токен стало бы
    можно одним GET на неаутентифицированный дашборд (вместо позиции MITM), а он
    же служит запасным ключом подписи обновлений агента."""
    client.app.state.ingest_token = "super-secret-ingest"
    client.app.state.admin_token = ""
    try:
        html = client.get("/").text
        assert "super-secret-ingest" not in html
    finally:
        client.app.state.ingest_token = ""


def test_admin_token_authorizes_delete_without_ingest_token(client) -> None:
    """Ревью блока D (H4): отдельный админ-секрет даёт работающие кнопки, не
    раскрывая ключ ingest и подписи обновлений."""
    client.app.state.ingest_token = "super-secret-ingest"
    client.app.state.admin_token = "admin-secret"
    try:
        html = client.get("/").text
        assert "admin-secret" in html  # в мету уходит ТОЛЬКО админ-секрет
        r = client.post(
            "/api/v1/devices/purge",
            json={"days": 30, "dry_run": True},
            headers={"x-srp-token": "admin-secret"},
        )
        assert r.status_code == 200
        bad = client.post("/api/v1/devices/purge", json={"days": 30, "dry_run": True})
