...
def test_net_changes_are_capped(tmp_path, monkeypatch) -> None:
    """o5-D12: журнал изменений топологии рос без предела — единственная таблица
    netdisco без обрезки (снапшоты топологии рядом обрезаются)."""
    from server import db

    db.init_db(str(tmp_path / "chg.db"))
    cap = 40  # потолок понижен: поведение то же, а тест не пишет 20 тыс. строк
    monkeypatch.setattr(db, "_retain_net_changes", cap)
    for i in range(cap + 50):
        db.store_net_change("appeared", device_nid=f"nd-{i}")

    with db._connect() as conn:
        n = conn.execute("SELECT COUNT(*) FROM net_changes").fetchone()[0]
        newest = conn.execute("SELECT device_nid FROM net_changes ORDER BY id DESC LIMIT 1")
        assert newest.fetchone()[0] == f"nd-{cap + 49}"  # свежие сохраняются
    assert n == cap
    assert db._retain_net_changes  # константа существует и на «боевом» значении
