...
# --------------------------------------------------------------------------- #
# A5: NdisPhysicalMedium collection (language-independent medium signal)      #
# --------------------------------------------------------------------------- #


def test_net_script_reads_ndis_physical_medium():
    assert "NdisPhysicalMedium" in network._NET_SCRIPT
    a = network._parse_adapter({"name": "Wi-Fi", "iftype": 71, "phys_medium": 9})
    assert a["phys_medium"] == 9


# --------------------------------------------------------------------------- #
# C3: an adapter no longer vanishes because of a link-speed cast overflow     #
# --------------------------------------------------------------------------- #


def test_net_script_guards_link_speed_overflow():
    assert "[uint64]$a.TransmitLinkSpeed" in network._NET_SCRIPT
    assert "} catch { continue }" in network._NET_SCRIPT
    assert "[int64]$a.TransmitLinkSpeed" not in network._NET_SCRIPT
...
def test_parse_route_keeps_internal_route_via_non_gateway_router():
    r = network._parse_route(
        {"dest": "10.20.0.0/16", "next_hop": "10.0.77.1", "if_index": 4, "metric": 10},
        set(),
    )
    assert r == {"dest": "10.20.0.0/16", "next_hop": "10.0.77.1", "if_index": 4, "metric": 10}
...
def test_collect_network_routes_only_counts_as_present(monkeypatch):
    ...
    only_r = {
        ...
        "connections": [],
        "quality": [],
        "routes": [{"dest": "10.20.0.0/16", "next_hop": "10.0.77.1", "if_index": 4, "metric": 10}],
    }
...
def test_ps_pings_only_rfc1918_targets():
    """Privacy (KodSR M7): the PS script must gate EVERY ping target (gateway AND
    adapter DNS) through the RFC1918 check BEFORE pinging -- a public resolver
    (8.8.8.8) gets no ICMP from the agent and does not eat one of the 4 target
    slots. Verified live on this machine 2026-08-23 (Test-Rfc1918: 10/8, 172.16/12,
    192.168/16 -> True; 8.8.8.8, 1.1.1.1, 172.32.0.1, 169.254.x, 100.64.x, v6 -> False).
    The Python-side _parse_quality filter stays as the second line of defence."""
    script = network._NET_SCRIPT
    fn = script.index("function Test-Rfc1918")
    targets = script.index("$targets = @()")
    ping = script.index("$pinger.Send(")
    assert fn < targets < ping
    assert "(Test-Rfc1918 $a.gateway)" in script
    assert "(Test-Rfc1918 $d)" in script
...
def test_lan_adapter_ips_keeps_only_lan_and_wifi_role():
    adapters = [
        ...
        {"role": "tunnel", "ipv4": ["10.0.77.2"]},  # e.g. an Outline endpoint -- RFC1918 too
...
_TUNNEL_ADAPTER = {
    ...
    "up": True,
    "link_bps": 0,
    "ipv4": ["10.0.77.2"],  # RFC1918 too -- must still be excluded via role, not IP
    "ipv6": [],
    "gateway": "",
    "dns": [],
    "dhcp": False,
...
def test_collect_network_active_scan_sweeps_lan_and_wifi_adapters_only(monkeypatch):
    ...
    assert seen == [["192.168.1.5"]]  # the tunnel adapter's 10.0.77.2 excluded
...
# --------------------------------------------------------------------------- #
# C4: ICMP measurements are time-bounded (no Test-Connection in WinPS 5.1)     #
# --------------------------------------------------------------------------- #


def test_net_script_uses_bounded_pinger():
    """o5-C4: Test-Connection в PS 5.1 не ограничить по времени (-TimeoutSeconds
    там нет) -- 4 недостижимые цели съедали весь бюджет run_ps и убивали ВЕСЬ
    сетевой срез. .NET Ping.Send(addr, ms) даёт явный таймаут на пробу."""
    assert "Test-Connection" not in network._NET_SCRIPT
    assert ".Send(" in network._NET_SCRIPT
