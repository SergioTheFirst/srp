...
def test_printer_ip_map_key_cardinality_capped(tmp_path) -> None:
    """o5-D5: у printer_ip_map нет обрезки по числу КЛЮЧЕЙ. Имя очереди приходит
    от агента, поэтому каждый конверт может принести свежие имена и таблица растёт
    без предела (та же дыра, что закрыта для disk_readings)."""
    from server import db

    db.init_db(str(tmp_path / "pim.db"))
    hints = [{"name": f"q{i}", "ip": "10.0.0.5"} for i in range(200)]
    db.store_printer_ip_hints("d1", hints)

    with db._connect() as conn:
        n = conn.execute("SELECT COUNT(*) FROM printer_ip_map WHERE device_id='d1'").fetchone()[0]
    assert n == db._MAX_PRINTER_KEYS_PER_DEVICE
