...
def test_identity_card_merges_three_tables(tmp_path: Path) -> None:
    ...
    assert card["ip"] == "192.168.77.50"  # добрано из net_devices (agent не знает IP)
...
def test_identity_card_by_mac_and_ip(tmp_path: Path) -> None:
    ...
    db.upsert_net_device({"device_nid": "nd-3", "ip": "192.168.77.51", "mac": "AA-BB-CC-DD-EE-03"})
    db.set_net_device_links("nd-3", device_id="dev-3")
    assert db.get_identity_card(mac="AA-BB-CC-DD-EE-03")["device_id"] == "dev-3"
    assert db.get_identity_card(ip="192.168.77.51")["device_id"] == "dev-3"


def test_identity_card_printer_only(tmp_path: Path) -> None:
    _seed(tmp_path)
    db.store_printer_reading(
        "prn-1", {"ip": "192.168.77.5", "hostname": "prn-host", "model": "LJ", "status": "online"}
    )
...
def test_fleet_does_not_duplicate_device_with_multiple_net_rows(tmp_path: Path) -> None:
    ...
    db.upsert_net_device(
        {"device_nid": "nd-4a", "ip": "192.168.77.99", "mac": "AA-00-00-00-00-04"},
        received_at="2026-07-01T09:00:00+00:00",  # older -> must lose
    )
    db.upsert_net_device(
        {"device_nid": "nd-4b", "ip": "192.168.77.61", "mac": "AA-00-00-00-00-05"},
    ...
    assert len(fleet) == 1
    assert fleet[0]["local_ip"] == "192.168.77.61"  # most-recently-seen net row wins
    ...
    assert card is not None and card["ip"] == "192.168.77.61"
...
def test_fleet_local_ip_falls_back_to_net_devices_when_no_historical(tmp_path: Path) -> None:
    ...
    db.upsert_net_device({"device_nid": "nd-5", "ip": "192.168.77.62", "mac": "AA-00-00-00-00-06"})
    db.set_net_device_links("nd-5", device_id="dev-5")
    row = next(d for d in db.get_devices() if d["device_id"] == "dev-5")
    assert row["local_ip"] == "192.168.77.62"
...
def test_operator_comment_survives_agent_envelope(tmp_path) -> None:
    """o5-D6: поле, редактируемое И агентом, И оператором, нуждается в
    детерминированном приоритете. Правило: правка оператора побеждает навсегда,
    пока оператор её не очистит (очистка возвращает управление агенту).
    Это фундамент блока F."""
    from server import db

    db.init_db(str(tmp_path / "d6.db"))
    did = "d6-dev"
    db.touch_device(did, "2026-03-01T10:00:00+00:00", "0.2.0", comment="from-agent")
    assert db.get_device(did)["comment"] == "from-agent"

    assert db.set_device_comment(did, "from-operator") is True
    db.touch_device(did, "2026-03-01T11:00:00+00:00", "0.2.0", comment="from-agent")
    assert db.get_device(did)["comment"] == "from-operator", "агент затёр правку оператора"

    # Очистка -- управление возвращается агенту НЕМЕДЛЕННО, ещё до нового конверта:
    # ревью блока D показало, что upsert копировал текст оператора в агентскую
    # колонку, и после очистки на дашборде продолжал висеть операторский текст.
    db.set_device_comment(did, "")
    assert db.get_device(did)["comment"] == "from-agent"

    db.touch_device(did, "2026-03-01T12:00:00+00:00", "0.2.0", comment="from-agent")
    assert db.get_device(did)["comment"] == "from-agent"
