...
def test_iface_speed_unknown_sentinel_becomes_none():
    # D6/KodSR L6: RFC 2863 ifSpeed=0xFFFFFFFF means "unknown", not 4295 Mbit/s.
    tables = {
        oids.IF_DESCR: {},
        oids.IF_TYPE: {},
        oids.IF_SPEED: {},
    ...
    row = _iftable(1, descr="eth0", if_type=6, speed_bps=0xFFFFFFFF, phys=None, oper=1)
    for full_oid, value in row.items():
        base = full_oid.rsplit(".", 1)[0]
        tables[base][full_oid] = value
    session = FakeSession(scalars={oids.SYS_DESCR: "sw"}, tables=tables)

    profile = snmp_probe.probe_device("10.0.0.13", session)

    assert profile.interfaces[0].speed_mbps is None
...
def test_dot1q_only_switch_sets_has_fdb():
    # VLAN-switch: empty dot1d-FDB, non-empty dot1q-FDB -- must still resolve has_fdb.
    session = FakeSession(
        scalars={oids.SYS_DESCR: "vlan-sw"},
        tables={
            oids.DOT1D_TP_FDB_PORT: {},
            oids.DOT1Q_TP_FDB_PORT: {f"{oids.DOT1Q_TP_FDB_PORT}.10.0.27.68.17.58.183": 5},
        },
    )
...
def test_no_bridge_still_walks_fdb_unconditionally():
    # o5-A1 conscious contract change: the `if bridge` gate on the FDB walk was
    # removed (an L3-switch can answer dot1q while hiding dot1dBaseBridgeAddress),
    # so both FDB tables are walked even when no bridge address was seen -- this
    # test used to assert the walk was SKIPPED; now it asserts it always happens.
    session = FakeSession(scalars={oids.SYS_DESCR: "host"})
    ...
    assert oids.DOT1D_TP_FDB_PORT in walked_bases
    assert oids.DOT1Q_TP_FDB_PORT in walked_bases
