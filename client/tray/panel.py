...
_OWNER_ROWS = (("full_name", "ФИО"), ("position", "Должность"), ("phone", "Телефон"))


def run_owner_form(*, config_path: Path) -> int:
    """Окно «SRP — Персональные данные». Возвращает 0 (отправлено/сохранено) или
    1 («Отмена», ничего не записано).

    По «Отправить»: валидация -> запись спула (``publish_owner_identity``) ->
    одна best-effort прямая отправка (``push_owner_identity``). Обе операции --
    F8/F11 -- гарантия доставки лежит на спуле + агенте, а не на этом окне.
    """
    import tkinter as tk
    from tkinter import messagebox, ttk
    ...
    prefill = read_own_spool()
    result = {"code": 1}
    root = tk.Tk()
    root.title("SRP — Персональные данные")
    frame = ttk.Frame(root, padding=16)
    frame.grid()

    entries: dict[str, tk.Entry] = {}
    for i, (key, label) in enumerate(_OWNER_ROWS):
        ttk.Label(frame, text=label + ":", width=12, anchor="w").grid(
            row=i, column=0, sticky="w", pady=4
        )
        entry = ttk.Entry(frame, width=32)
        entry.insert(0, prefill.get(key, ""))
        entry.grid(row=i, column=1, pady=4)
        entries[key] = entry

    def submit() -> None:
        # .strip() ДО валидации И до отправки: валидация и так считала длину по
        # обрезанному значению, а наружу уходило сырое -- 140 знаков плюс пробел
        # давали 422 на контракте при показанном пользователю «сохранено».
        full_name, position, phone = (entries[k].get().strip() for k, _ in _OWNER_ROWS)
        error = validate_owner_fields(full_name, position, phone)
        if error:
            messagebox.showerror("SRP", error)
            return
        publish_owner_identity(full_name, position, phone)
        sent = push_owner_identity(config_path, full_name, position, phone)
        messagebox.showinfo(
            "SRP",
            "Отправлено на сервер" if sent else "Сохранено, уйдёт на сервер при ближайшей связи",
        )
        result["code"] = 0
        root.destroy()
    ...
    btns.grid(row=len(_OWNER_ROWS), column=0, columnspan=2, sticky="e", pady=(14, 0))
    ttk.Button(btns, text="Отправить", command=submit).grid(row=0, column=0, padx=4)
    ttk.Button(btns, text="Отмена", command=cancel).grid(row=0, column=1, padx=4)
    root.bind("<Return>", lambda _evt: submit())
    root.mainloop()
    return result["code"]
