...
Modes:
  ...
  * ``--edit-owner`` -- show the owner-identity form (o5 блок F, F11), then exit
...
# Потолок одного schtasks/taskkill: обе команды отрабатывают доли секунды, а
# ждать их приходится ТРИ раза подряд плюс _AGENT_STOP_TIMEOUT_SEC -- при 60 с
# пользователь остался бы без иконки и без ответа на две с лишним минуты.
_STOP_CMD_TIMEOUT_SEC = 20
...
def _int_field(data: dict, key: str, default: int) -> int:
    """Per-field fallback: a non-numeric value (KodSR L1, e.g. a stray "abc")
    must degrade to *default*, not crash the whole tray at startup."""
...
def _str_bool(value: object) -> bool:
    """String-aware bool: ``bool("false")`` is True in plain Python -- config.json
    written by hand (or by an older tool) can carry the string "false" and would
    otherwise silently flip the flag on (KodSR L1)."""
...
def _read_cert_config(config_path: Path) -> tuple[int, int, bool, str]:
    ...
    or ACL-blocked (the tray runs as an ordinary user), or when individual fields
    hold a malformed value instead of the file being absent entirely.
    """
    try:
        data = json.loads(config_path.read_text(encoding="utf-8"))
    except (OSError, ValueError):
    ...
    if not isinstance(data, dict):
        data = {}
    return (
        _int_field(data, "tray_cert_warn_days", 14),
        _int_field(data, "tray_notify_hours", 4),
        _str_bool(data.get("tray_require_cert", False)),
...
class _TrayApp:
    ...
    def open_owner_form(self) -> None:
        subprocess.Popen(self._child("--edit-owner"), creationflags=NO_WINDOW)  # nosec B603
...
def _alert(text: str) -> None:
    ...
    ctypes.WinDLL("user32", use_last_error=True).MessageBoxW(None, text, "SRP", 0x30)
...
def run_stop_agent(
    ...
    try:
        ...
        # Публичная сборка: client/deploy/ (инсталлятор + SYSTEM-задача) не
        # поставляется, поэтому и останавливать нечего -- команды остановки
        # знает именно он. Полная версия модуль содержит: поведение прежнее.
        alert("Остановка агента недоступна в этой сборке.")
        return 1

    def _default_runner(cmd: list[str]) -> int:
        # фиксированный argv, собранный строкой выше -- ни shell, ни ввода юзера.
        # Таймаут обязателен: этот код бежит в невидимом elevated-ребёнке (SW_HIDE),
        # и зависший schtasks/taskkill оставлял висеть процесс, который никогда не
        # дойдёт до проверки разблокировки -- агент не остановлен, а сказать об
        # этом пользователю уже некому (родитель закрыл иконку). С границей ребёнок
        # честно доходит до MessageBox «не удалось остановить».
        try:
            proc = subprocess.run(  # nosec B603
                cmd, capture_output=True, creationflags=NO_WINDOW, timeout=_STOP_CMD_TIMEOUT_SEC
            )
        except (OSError, subprocess.SubprocessError):
            return 1  # код игнорируется вызывающим: честный сигнал -- разблокировка EXE
...
def _acquire_single_instance() -> Optional[int]:
    ...
    # use_last_error=True -- иначе GetLastError() всегда 0 и ERROR_ALREADY_EXISTS
    # не виден; restype=HANDLE -- дефолтный c_int обрезает 64-битный хэндл.
    k32 = ctypes.WinDLL("kernel32", use_last_error=True)
    k32.CreateMutexW.argtypes = [ctypes.c_void_p, wintypes.BOOL, wintypes.LPCWSTR]
    k32.CreateMutexW.restype = wintypes.HANDLE
    handle = k32.CreateMutexW(None, False, _MUTEX_NAME)
    # get_last_error(), а не k32.GetLastError(): при use_last_error=True реальный
    # системный код лежит в приватной копии ctypes.
    err = ctypes.get_last_error()
    if st.single_instance_should_exit(err):
        return None
    if not handle:
        # NULL от CreateMutexW (чужой сеанс/имя занято другим объектом): трей должен
        # подняться без защиты от второго экземпляра, а не упасть на int(None).
        log.error("CreateMutexW failed (%d) -- single-instance guard off", err)
        return 0
    return int(handle)
...
def _parse_args(argv: Optional[list[str]]) -> argparse.Namespace:
    ...
    p.add_argument(
        ...
        action="store_true",
        dest="edit_owner",
        help="показать окно «Персональные данные» и выйти (o5 блок F)",
    )
    p.add_argument(
        "--stop-agent",
        action="store_true",
...
def main(argv: Optional[list[str]] = None) -> int:
    ...
    if args.panel:
        ...
        return 0
    if args.edit_owner:
        return panel.run_owner_form(config_path=config_path)
