...
class ClientConfig:
    ...
    # Персональные данные владельца ПК. Приходят из трей-спула В ПАМЯТЬ на каждом
    # цикле (Agent._refresh_owner_identity) и агентом на диск НЕ пишутся: каталог
    # установки закрыт на запись для Users, а save_config требует пароль (F.0).
    owner_full_name: str = ""
    owner_position: str = ""
    owner_phone: str = ""
    ...
    update_channel: str = "stable"  # "stable" (on) | "none" (off) -- no other channel exists
