...
registered name (confirmed present via ``nbtstat -n``: it lists <00>/<20>)
received no reply. The Windows-native ``nbtstat -A <ip>`` tool DOES get
answers -- live-verified against several real Windows hosts on a real LAN
(three of three resolved). The agent is always Windows, so ``nbtstat`` is
always available; this module now shells out to it instead of the raw socket.
