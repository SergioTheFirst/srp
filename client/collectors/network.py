...
def _parse_adapter(raw: Any) -> Optional[dict[str, Any]]:
    ...
    return {
        ...
        "kind": kind,
        "phys_medium": _i(raw.get("phys_medium")),
        "role": role,
        "tunnel": tunnel,
...
foreach ($a in Get-NetAdapter) {
  try {
    $c = $cfg["$($a.ifIndex)"]
    $bps = 0
    try { $bps = [uint64]$a.TransmitLinkSpeed } catch { $bps = 0 }
    # F7: rough anti-overflow clamp only (uint64 garbage/wraparound); the precise
    # implausible-speed cutoff lives in this file's own _parse_adapter
    # (_SPEED_MAX_PLAUSIBLE_BPS) -- agent-side, the server does not re-validate it.
    if ($bps -gt 1000000000000000) { $bps = 0 }
    $adapters += [ordered]@{
      name     = "$($a.Name)"
      desc     = "$($a.InterfaceDescription)"
      mac      = "$($a.MacAddress)"
      iftype   = [int]$a.InterfaceType
      phys_medium = [int]$a.NdisPhysicalMedium
      up       = ($a.Status -eq 'Up')
      link_bps = [int64]$bps
...
# Privacy (KodSR M7): ping ONLY RFC1918 literals. A public resolver (8.8.8.8,
# 1.1.1.1) is a routine adapter DNS setting -- it must get no ICMP from the agent
# and must not eat one of the 4 target slots. Same 10/8, 172.16/12, 192.168/16
# contract as the Python-side _is_internal filter (which still drops anything
# that slips through before it reaches the envelope).
...
foreach ($a in $adapters) {
  if ($a.gateway -and (Test-Rfc1918 $a.gateway)) {
    $targets += [pscustomobject]@{ kind='gateway'; addr=$a.gateway }
  }
  foreach ($d in $a.dns) {
    if ($d -and (Test-Rfc1918 $d)) { $targets += [pscustomobject]@{ kind='dns'; addr=$d } }
  }
}
# Dedupe keeping insertion order (gateways first), cap at 4 targets. WinPS 5.1
# has no way to bound the ping cmdlet in time, so we use the .NET pinger with an
# explicit per-probe timeout: worst case 4 targets * 3 probes * 1s = 12s, well
# under the 60s run_ps cap. Ping only literal IPs (no name resolution).
$seen = @{}
$uniq = @()
...
foreach ($tg in $targets) {
  try {
    ...
    for ($k = 0; $k -lt 3; $k++) {
      try {
        $rep = $pinger.Send($tg.addr, 1000)
        if ($rep.Status -eq 'Success') { $lats += [double]$rep.RoundtripTime }
      } catch {}
    }
    $recv = $lats.Count
    $lat = $null
    if ($recv -gt 0) { $lat = [math]::Round((($lats | Measure-Object -Average).Average), 1) }
    $quality += [ordered]@{
      target_kind = $tg.kind
      target      = $tg.addr
      latency_ms  = $lat
      loss_pct    = [math]::Round(((3 - $recv) / 3.0) * 100, 1)
      samples     = 3
    }
  } catch { continue }
}
...
# ICMP quality block budgets up to ~12s) right after an active sweep,
