...
1. `NOT_APPLICABLE` — легитимно неприменим (battery@desktop). **Не деградация**, домен не существует.
...
| Battery | BatteryInfo | — · `NOT_APPLICABLE` если chassis=desktop/present=false |
...
- cross-field консистентность (battery present без design; SSD media_type с HDD-only полями) — stateless
...
- desktop → `battery = NOT_APPLICABLE`, без шумового UNKNOWN.
