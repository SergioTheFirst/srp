...
- **Целостность+аутентичность**: sha256 обязательно; при заданном `ingest_token`
  ...
  токена не подделает пакет. Агент с токеном ТРЕБУЕТ hmac (fail-closed).
  Downgrade-replay отрезан правилом «применять только строго большую версию».
  **[Исправлено по security-review, HIGH]**: без токена sha256 — это только
  целостность, не подлинность; LAN-MITM мог бы самоподписать вредоносный пакет
  и получить SYSTEM RCE на всём парке через `setup.exe --update`. Агент БЕЗ
  `ingest_token` теперь вообще не скачивает/не применяет пакет — только
  сообщает «доступна версия X» на дашборд (`client/updater.py::Updater.check`,
  гейт сразу после dev-build проверки). Автоприменение включается ТОЛЬКО когда
  токен настроен на обеих сторонах — тогда HMAC-проверка обязательна везде,
  где реально исполняется код.
