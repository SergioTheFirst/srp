...
- Explicit Phase-1 gaps (from branch review, accepted): the spec's "configured SRP-server address counts as internal" exception is NOT implemented — a public-IP server connection edge is dropped (needs the same `server_url` plumbing; security-review Important #2). Per-block `partial` collector status is unreachable — Phase 1 runs ONE PowerShell script all-or-nothing, so a policy-blocked single cmdlet shows as an empty block inside an `ok` result (spec §5.4 deviation; code-review Important #2). Neighbor broadcast filtering is MAC-based only — an RFC1918 directed-broadcast IP with a unicast MAC passes (needs prefix length, not emitted; code-review Important #1).
```