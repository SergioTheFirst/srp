...
class HealthVerdict:
    ...
    # Последнее состояние, отличное от "unknown". Нужно храповику, чтобы цикл
    # «нет видимости» не стирал память (см. _ratchet). Аддитивно.
...
def _state(d: Coordinate, r: Coordinate, o_val: float, flags: set) -> tuple[str, str, list]:
    ...
    # observability, OR отсутствующая координата УЩЕРБА, независимо от o_val.
    # Ревью блока E: раньше гейт требовал, чтобы обе координаты были None, но
    # Resilience почти всегда жив (aging/trajectory) -- поэтому скрытый Damage
    # (o5-E1 погасил ось хранилища/ОС) проваливался мимо проверки h1 и машина в
    # предотказе выдавалась за «здоров». Нет канала ущерба -- нет вердикта.
    if o_val < 40 or d.value is None:
...
def _ratchet(
    ...
    if prev_state == "unknown":
        # Цикл «нет видимости» не должен СТИРАТЬ память храповика: иначе машина,
        # у которой источник ущерба на один цикл потерял доверие, на следующем
        # цикле объявляется здоровой. Берём последнее известное состояние.
        prev_state = prev.get("last_known_state")
...
def _reconcile(state: str, index: Optional[float], o_val: float) -> tuple[str, list]:
    # state == "unknown" тоже: индекс считается по одной уцелевшей координате,
    # и на цикле «нет видимости» цвет флипался с bad на good.
    if o_val < 40 or state == "unknown":
        return "unknown", []
...
def compute_health(
    ...
    last_known = state if state != "unknown" else (prev_health or {}).get("last_known_state")
