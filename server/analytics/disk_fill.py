...
def compute_disk_fill_risk(
    ...
    *,
    ...
    gated = domain_state == "unknown"
    if gated:
        # Ревью блока E: гейт домена гасит ТОЛЬКО свой сигнал. Свободное место
        # приходит из источника free_space, а сбои обслуживания Windows -- из
        # журнала событий (другой источник), поэтому тревога «машина не
        # обновляется» обязана пережить недоверие к свободному месту.
        samples = []
        if not collapse:
            return make_score100(
                None,
                ...
                "unknown",
                "unknown",
                missing_evidence=["источник disk_fill не прошёл проверку доверия"],
                source_lineage={"disk_fill_domain": "unknown"},
                reason="источник свободного места не прошёл гейт доверия — оценка скрыта",
            )
    ...
    if gated:
        # Не «не наблюдалось» -- наблюдалось, но не прошло гейт доверия. Смешивать
        # эти два случая нельзя: оператор иначе не отличит «места не видно» от
        # «месту не верим» (повторное ревью блока E).
        missing.append("свободное место не прошло гейт доверия (значение не учтено)")
    elif not samples:
    ...
    return make_score100(
        ...
        source_lineage={
            # Маркер гейта -- та же конвенция, что в storage.py/os_degradation.py.
            **({"disk_fill_domain": "unknown"} if gated else {}),
