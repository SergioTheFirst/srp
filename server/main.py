...
def _run_retention_sweep(cfg: ServerConfig) -> None:
    ...
    and net-map ghosts that finished their eligible_purge grace period (D3).

    F7 (code-review MEDIUM): the two purges are independent -- the net-map ghost
    purge has its own 30-day ``eligible_purge`` criterion, unrelated to
    ``device_retention_days`` -- so it runs first and unconditionally, never
    gated behind (or short-circuited by a failure in) the agent-device purge.
    ...
    """
    try:
        net_result = db.purge_net_devices_eligible()
    except Exception:  # never let a transient sweep error crash the caller
        _log.exception("net device purge sweep failed")
    else:
        if net_result["count"]:
            _log.info(
                "retention sweep deleted %d eligible net device(s): %s",
                net_result["count"],
                ", ".join(net_result["device_nids"]),
            )
...
def _run_maintenance_sweep(cfg: ServerConfig) -> None:
    ...
    try:
        ...
        db.prune_aged(
            ...
            print_jobs_raw_days=cfg.print_jobs_raw_days,
            scores_raw_days=cfg.scores_raw_days,
            historical_raw_days=cfg.historical_raw_days,
        )
        db.run_maintenance()
...
def create_app(cfg: ServerConfig | None = None) -> FastAPI:
    ...
    async def lifespan(_: FastAPI):
        db.init_db(
            ...
            retain_historical=cfg.retain_historical,
        ...
        if not limits.DEMO_MODE:
            # Демо-база собрана сидером заранее -- стартовые свипы (backfill/
            # ретеншн/rollup) её не трогают, только реальные фермы.
        ...
        # review: DEMO_MODE excluded here too -- this loop re-runs the very sweeps
        # the startup skip above prevents (incl. device deletion via
        # purge_devices_silent_for); without this, a long-running demo would only
        # have delayed the deletion by one purge_interval_hours, not avoided it.
        if cfg.purge_interval_hours > 0 and not limits.DEMO_MODE:
            tasks.append(asyncio.create_task(_retention_loop(cfg)))
        # review: DEMO_MODE excluded too -- staleness re-eval persists trust state
        # (a write), and the demo park's timestamps are seeded up to "now" at
        # container start: left running, this loop would steadily mark every
        # source stale and rot the showcase into UNKNOWN hours after deploy.
        if cfg.source_stale_reeval_interval_sec > 0 and not limits.DEMO_MODE:
            tasks.append(asyncio.create_task(_source_staleness_loop(cfg)))
        # review: read-only demo must not depend on server/config.json shipping with
        # printers/netdisco off (Task 15) -- SRP_DEMO=1 alone must be enough. The
        # seeder writes printer/net_* rows directly; none of these loops are needed.
        if not limits.DEMO_MODE:
    ...
    # Отдельный админ-секрет для кнопок дашборда: он и только он попадает в HTML.
    app.state.admin_token = cfg.admin_token
    ...
    elif not cfg.admin_token:
        # Без admin_token кнопки удаления/очистки на дашборде получают 401 и
        # оператору неоткуда узнать, что для них есть ОТДЕЛЬНЫЙ ключ (ingest_token
        # в HTML не публикуется намеренно -- дашборд не аутентифицирован).
        _authlog.warning(
            "admin_token не задан — кнопки удаления и очистки машин на дашборде "
            "будут отвечать «отказано». Задайте admin_token в server/config.json "
            "(отдельный от ingest_token), чтобы они заработали."
        )
    ...
    # whether to start the periodic printer loop at all.
    app.state.printer_poll_enabled = cfg.printer_poll_enabled
    # F5 (2026-08-23 review): the netdisco kill-switch must gate the "собрать
    # карту сейчас" button too, not just the periodic background loops above --
    # an operator who has netdisco off must not have the button run an active
    # scan anyway.
    app.state.netdisco_enabled = cfg.netdisco_enabled
