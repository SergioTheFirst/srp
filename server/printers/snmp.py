...
_GETBULK_MAX_REPETITIONS = 25


def _oid_key(o: str) -> tuple:
    """Числовой ключ сравнения OID: "1.3.6.…9" < "…10" по значению arc, а не по
    символам строки (F3 — лексикографическое ">" откатывало курсор назад на
    каждой границе разрядов, например "…2.9" > "…2.10" строкой)."""
...
def _walk_pdu(
    ...
    *,
    ...
    """Собрать PDU очередного шага обхода: GETBULK (v2c, до
    _GETBULK_MAX_REPETITIONS строк, подрезано остатком бюджета ``remaining``)
    либо GETNEXT (v1 или откат после пустого GETBULK-ответа)."""
    if use_getbulk:
        max_repetitions = min(_GETBULK_MAX_REPETITIONS, remaining)
...
def snmp_walk(
    ...
    *,
    ...
    """Обойти табличную базу OID. Ключ = полный OID.

    v2c (version != 0) шлёт GETBULK с non_repeaters=0 и max_repetitions до
    _GETBULK_MAX_REPETITIONS — до 25 строк за один round-trip вместо одной
    (было: GETNEXT по одному варбайнду). v1 (version == 0) не знает GETBULK →
    остаётся на GETNEXT. Если агент прислал пустой ответ на GETBULK (часть
    агентов его отвергает), откатываемся на GETNEXT с той же позиции —
    надёжность на недоверенном вводе не теряется, GETBULK — best-effort
    ускорение, а не обязательное условие корректности.

    Останавливается при выходе за префикс base_oid, отсутствии прогресса
    (защита от зацикливания на битом агенте) или лимите max_rows строк
    РЕЗУЛЬТАТА (раньше лимитом было число GETNEXT-транзакций, что совпадало с
    числом строк 1:1; с GETBULK один ответ приносит пачку строк, поэтому
    max_repetitions каждого запроса подрезается остатком бюджета — объём
    данных, запрошенных за сеть, не превышает прежний потолок). Только чтение.
    """
    result: Dict[str, object] = {}
    current = base_oid
    prefix = base_oid + "."
    use_getbulk = version != 0
    while len(result) < max_rows:
        ...
        pkt = _walk_pdu(
            ...
            version=version,
            request_id=request_id,
            use_getbulk=use_getbulk,
            remaining=max_rows - len(result),
        )
        parsed = _transact(host, port, pkt, timeout, retries, request_id)
        if not parsed:
            if use_getbulk:
                use_getbulk = False  # агент отверг GETBULK → откат на GETNEXT, тот же current
                continue
            break
        next_current = None
        left_subtree = False
        for oid, value in parsed.items():
            if len(result) >= max_rows:  # недоверенный агент прислал больше max_repetitions
                break
            if oid == base_oid or oid.startswith(prefix):
                ...
                result[oid] = value
                try:
                    is_newer = next_current is None or _oid_key(oid) > _oid_key(next_current)
                except ValueError:
                    # враждебный/битый OID с нечисловым сегментом -- не роняем
                    # обход, просто не двигаем курсор по этой строке (F3)
                    is_newer = False
                if is_newer:
                    next_current = oid
            else:
