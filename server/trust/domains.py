...
def resolve_domain_trust(domain: str, sources: Dict[str, SourceTrust]) -> DomainTrust:
    """Свести доверие источников домена в одно состояние (гейт) и вес (модуляция).

    Про `weight` (o5-E5): он вычисляется и сохраняется, но НИ ОДИН потребитель его
    пока не применяет -- все читают только `state`. Это осознанно отложенная
    модуляция, а не забытая проводка: вес не имеет права воскрешать источник,
    проваливший гейт (CLAUDE.md §5), а калибровки для его применения ещё нет.
    """
