...
def _gate_axis(
    *,
    ...
    # o5-E4: NOT_APPLICABLE обязательного домена -- это «датчика нет на этом
    # оборудовании», а не «всё хорошо»: уверенное значение по отсутствующему
    # источнику и есть ложная уверенность. Обрабатываем так же, как unknown.
    # Локальный импорт: этот модуль определяет только ФОРМУ оценки, а
    # server.trust.domains тянет за собой слой БД (trust -> staleness -> db).
    # Держим зависимость внутри функции, чтобы модуль-форма оставался лёгким.
    from server.trust.domains import DomainTrustState

    required_na = [
        n for n in required if _domain_state(domains, n) == DomainTrustState.NOT_APPLICABLE.value
    ]
    missing = [f"{n} недоступен" for n in (required_unknown + optional_unknown)]
    missing += [f"{n}: датчик отсутствует на этом оборудовании" for n in required_na]

    if required_unknown or required_na:
        if is_risk:
            return make_score100(
                numeric,
                direction,
