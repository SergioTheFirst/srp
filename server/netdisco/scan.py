...
from server.printers.scan import drop_saturated, expand_cidrs, local_cidrs
...
def scan(
    ...
    *,
    ...
    on_saturated: Optional[Callable[[List[str]], None]] = None,
) -> List[str]:
    ...
    without touching the network. ``on_saturated`` (F6/MEDIUM-2 review), if
    given, is called ONCE with the dropped /24 range keys (e.g. ``"10.33.0"``)
    when ``drop_saturated`` discards at least one -- lets the caller surface the
    guard to the operator instead of only the scan-module log line.
    """
    ...
    with ThreadPoolExecutor(max_workers=workers) as pool:
        for ip, ok in zip(hosts, pool.map(check, hosts)):
            if ok:
                found.append(ip)
    kept, dropped = drop_saturated(hosts, found)  # VPN/proxy answering a whole /24 -> not devices
    if dropped and on_saturated is not None:
        on_saturated(dropped)
    return kept
