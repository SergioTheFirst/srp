...
class UniFiAdapter(NetworkAdapter):
    ...
    def _build(
        ...
        for row in clients:
            ...
            if (
                row.get("is_wired") is False
                and isinstance(ap_mac, str)
                and ap_mac
                and len(links) < _MAX_LINKS
            ):
                # The controller knows this client is on radio -- a fact SNMP/FDB
                # can't see. Emit it directly rather than waiting on inference.
                links.append(
                    AdapterLink(
                        a_mac=slot["mac"], b_mac=ap_mac, link_kind="wireless", medium="wireless"
                    )
                )
        nodes = tuple(
            AdapterNode(
