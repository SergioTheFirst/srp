...
def collect_wireless(session: Session, *, sys_object_id: Optional[str]) -> List[LinkEvidence]:
    ...
    # ponytail: sys_object_id can arrive as a live SNMP GET from the untrusted
    # device itself (A6) -- the decoder's return type is whatever tag the
    # device sent (int for INTEGER, str for OCTET STRING), never guaranteed.
    if not isinstance(sys_object_id, str) or not sys_object_id:
        return []
