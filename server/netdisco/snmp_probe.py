...
_MAC_OCTETS = 6
_IF_SPEED_UNKNOWN = 0xFFFFFFFF  # RFC 2863: ifSpeed reports this when the true speed is unknown
...
def _build_interfaces(session: Session) -> tuple[NetInterface, ...]:
    ...
    for idx in indices:
        ...
        if bps == _IF_SPEED_UNKNOWN:  # D6/KodSR L6: "unknown", never 4295 Mbit/s
            bps = None
        if_alias = (_str(alias.get(idx)) or "")[:_IF_ALIAS_MAX] or None
        # review B1-B5 final LOW: same hygiene the B5 review fix (1f74401) gave
        # sys_descr/model_name -- ifAlias is free text from an untrusted
        # SNMP-answering host too; a hostile responder could otherwise plant
        # control bytes / a bidi-override and have it render verbatim on the
        # net-device card (autoescape stops XSS, not glyph-order spoofing).
        if if_alias is not None and not if_alias.isprintable():
            if_alias = None
        out.append(
            NetInterface(
                if_index=int(idx),
...
def probe_device(ip: str, session: Session, *, is_printer_fn=is_printer) -> DeviceProfile:
    ...
    # VLAN-свитчи отдают пустой dot1d-FDB и заполняют dot1q (evidence.collect_evidence
    # уже предпочитает dot1q -- классификатор обязан видеть тот же сигнал). Условие
    # `if bridge` снято: L3-свитч часто отвечает dot1q, скрывая dot1dBaseBridgeAddress.
    has_fdb = bool(session.walk(oids.DOT1Q_TP_FDB_PORT, max_rows=_FDB_PROBE_ROWS)) or bool(
        session.walk(oids.DOT1D_TP_FDB_PORT, max_rows=_FDB_PROBE_ROWS)
    )
    ...
    if sys_descr is not None and not sys_descr.isprintable():
        sys_descr = None  # same hygiene as model_name: drop control bytes/bidi-override
