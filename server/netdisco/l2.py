...
def infer_edges(
    ...
    *,
    ...
    A7: ``radio_ifindexes`` are this device's own radio interfaces (ifType 71,
    already read off its ifTable -- no extra SNMP call). An FDB edge whose port
    resolves to one of them was learned off an AP's radio, not a wire, so it is
    tagged ``medium="wireless"``; every other edge keeps ``medium=None`` (A4 --
    absence of medium is no longer read as "wired")."""
    ...
    for port, raw in port_macs.items():
        ...
        medium = "wireless" if ifx is not None and ifx in radio_ifindexes else None
        ...
        if infra_hits or len(macs) > UPLINK_MAC_THRESHOLD:
            ...
            for mac in infra_hits:
                tagged.append(
                    (port, LinkEvidence(local, mac, SOURCE_FDB_UPLINK, LOW, ifx, medium=medium))
                )
        elif len(macs) == 1:
            (mac,) = tuple(macs)
            tagged.append(
                (port, LinkEvidence(local, mac, SOURCE_FDB_EDGE, HIGH, ifx, medium=medium))
            )
        else:  # 2..threshold non-infra MACs -> hub/unmanaged switch behind the port
            for mac in macs:
                tagged.append(
                    (port, LinkEvidence(local, mac, SOURCE_FDB_AMBIGUOUS, LOW, ifx, medium=medium))
                )
