...
_DEFAULT_MISSING_AFTER_SEC = 6 * 3600  # D2: >= the agents' 4h historical cadence + margin,
# so an ARP-only host does not flap to "missing" between two snapshots
...
class NetdiscoConfig:
    ...
    # D2: a device with no real evidence (probe answer / SNMP / agent ARP snapshot)
    # for this long goes "missing" -- replaces the old 3x-topology-interval guess.
    missing_after_sec: int = _DEFAULT_MISSING_AFTER_SEC
...
def load_netdisco_config(data: Optional[Mapping[str, Any]]) -> NetdiscoConfig:
    ...
    missing_after = max(
        _MIN_INTERVAL_SEC,
        _as_int(d.get("missing_after_sec"), _DEFAULT_MISSING_AFTER_SEC),
    )
    ...
    return NetdiscoConfig(
        ...
        missing_after_sec=missing_after,
        jitter_sec=jitter,
