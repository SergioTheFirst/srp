...
# F7: same values as _PRINTER_UPGRADABLE, but a separate constant -- two unrelated
# upgrade policies (printer-FK vs agent-route gateway) must not be tied together
# by sharing one name; a future change to one must not silently change the other.
_GATEWAY_UPGRADABLE = ("unknown", "endpoint")
...
# NDIS physical medium -> среда. Числовые коды NDIS_PHYSICAL_MEDIUM.
_WIRELESS_PHYS_MEDIA = (1, 8, 9, 12)
_WIRED_PHYS_MEDIA = (14,)


def _medium_for_adapter(kind: Optional[str], phys_medium: Optional[int] = None) -> str:
    if phys_medium in _WIRELESS_PHYS_MEDIA:
        return "wireless"
    k = (kind or "").lower()
    if any(w in k for w in _WIRELESS_KINDS):
        return "wireless"
    if phys_medium in _WIRED_PHYS_MEDIA or k == "ethernet":
        return "wired"
    return "unknown"


_AP_CLIENT_TYPES = ("endpoint", "unknown", "phone")
_WIRED_SOURCES = ("lldp", "cdp")


def _medium_for_link(link: dict[str, Any], nodes: dict[str, _Node]) -> str:
    """Ф2-эвристика без fail-open: l3 по link_kind; AP<->клиент -> wireless;
    стандартный сосед (LLDP/CDP) на медном порту -> wired; иначе unknown."""
    if "l3" in (link.get("link_kind") or "").lower():
        return "l3"
    ends = [
        nodes.get(e) if isinstance(e, str) else None for e in (link.get("a_nid"), link.get("b_nid"))
    ]
    types = [(n or {}).get("dev_type") for n in ends]
    if types.count("ap") == 1:
        other = types[1] if types[0] == "ap" else types[0]
        if other in _AP_CLIENT_TYPES:
            return "wireless"
    if (link.get("via_source") or "").lower() in _WIRED_SOURCES:
        return "wired"
    return "unknown"
...
def _is_unplaceable_ip(ip: Optional[str]) -> bool:
    # nosec B104 -- string comparison against an agent-reported adapter IP, not a
    # bind address; bandit's "bind all interfaces" heuristic false-positives here.
    return not ip or ip == "0.0.0.0" or ip.startswith("169.254.")  # nosec B104


def _agent_nid(snap: dict[str, Any], by_mac: _Index, by_ip: _Index) -> str:
    """A11: scan every adapter (not just the primary) for a MAC/IP this agent already
    owns a node under, so a multi-NIC agent whose primary adapter differs from the one
    net_devices/ARP already knows never doubles the node. Deterministic: adapters are
    tried in snapshot order; a fresh nid is minted only when none of them resolve.

    F5: a virtual/tunnel/down adapter is skipped entirely -- these give the SAME
    address on many different machines (VirtualBox host-only 192.168.56.1, Docker
    172.17.0.1, shared Npcap/MS-Loopback MACs), so a match there would steal an
    unrelated node's identity for two different real agents (worse than the
    doubling this loop was written to fix). A link-local/unset IP is likewise
    never a usable identity key."""
    for a in snap.get("adapters") or []:
        if not isinstance(a, dict):
            continue
        if (
            a.get("tunnel") is True
            or a.get("role") in ("tunnel", "virtual")
            or a.get("up") is not True
        ):
            continue
        nm = normalize_mac(a.get("mac"))
        if nm and nm in by_mac:
            return by_mac[nm]
        ip = (a.get("ipv4") or [None])[0]
        if not _is_unplaceable_ip(ip) and ip in by_ip:
            return by_ip[ip]
    a0 = _primary_adapter(snap)
    ip0 = (a0.get("ipv4") or [None])[0]
    return _resolve_nid(a0.get("mac"), ip0, by_mac, by_ip)
...
def _merge_agents(
    ...
    for snap in snapshots:
        ...
        nid = by_device_id.get(did) or _agent_nid(snap, by_mac, by_ip)
        if nid == "nd-unknown":
...
def _gateways(snap: dict[str, Any]) -> dict[str, tuple[Optional[str], Optional[int], bool]]:
    """gateway-IP -> (kind, phys_medium, is_tunnel) of the adapter carrying it (medium hint).
    ...
    out: dict[str, tuple[Optional[str], Optional[int], bool]] = {}
    ...
    for a in sorted(adapters, key=lambda a: a.get("up") is not True):
        ...
        if gw and str(gw) not in out:
            is_tunnel = a.get("tunnel") is True or a.get("role") == "tunnel"
            out[str(gw)] = (a.get("kind"), a.get("phys_medium"), is_tunnel)
    return out
...
def _ensure_gateway(
    ...
    if existing is not None:
        if existing.get("dev_type") in _GATEWAY_UPGRADABLE:
            # ARP-дефолт "endpoint" -- не классификация: быть next-hop'ом в таблице
            # маршрутов агента строго сильнее. Инфра-типы (switch/ap/router/server)
            # не трогаем.
...
def _agent_uplinks(
    ...
    for snap in snapshots:
        ...
        for gw, (kind, phys_medium, is_tunnel) in _gateways(snap).items():
                ...
                links.append(
                    {
                        ...
                        "confidence": "high",
                        "ambiguous": False,
                        "medium": "l3" if is_tunnel else _medium_for_adapter(kind, phys_medium),
                        "vlan": None,
                        "a_port": None,
                        "b_port": None,
                        "speed_mbps": None,
