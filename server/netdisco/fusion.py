...
def _conflicted_ports(evidence: List[LinkEvidence]) -> set:
    """Ports (local node, port_space, local_if) whose top-priority source names >1
    remote node.
    ...
    ``port_space`` keeps LLDP's bridge-port numbering and FDB/CDP's ifIndex
    numbering apart (A9): a coincidental digit match across the two spaces is not
    the same physical port and must never be treated as a conflict."""
    by_port: Dict[Tuple[str, str, int], List[LinkEvidence]] = defaultdict(list)
    for ev in evidence:
        node_a, node_b = _node_id(ev.a), _node_id(ev.b)
        if ev.local_if is not None and node_a != node_b:
            by_port[(node_a, ev.port_space, ev.local_if)].append(ev)
    conflicted = set()
...
def fuse(evidence: List[LinkEvidence]) -> List[ResolvedLink]:
    ...
    for pair, evs in groups.items():
        ...
        ambiguous = any(
            e.local_if is not None and (_node_id(e.a), e.port_space, e.local_if) in conflicted
            for e in evs
        )
        ...
        # ifIndex -- writing it into a_if/b_if would make the unified assembler join
        # net_interfaces by the wrong numbering space (wrong port speed/state). Only
        # a winner in the ifIndex space may populate a_if; b_if is never populated.
        a_if = winner.local_if if winner.port_space == "ifindex" else None
        b_if = None
        ...
        # Никакого fail-open: не знаем среду -- не утверждаем (UNKNOWN над ложной
        # уверенностью).
