...
def _tarjan(adjacency: Dict[str, FrozenSet[str]]) -> "tuple[Set[str], Set[FrozenSet[str]]]":
    """One O(V+E) DFS pass computing cut vertices + bridges via Tarjan disc/low numbering.
    Iterative (explicit stack, no recursion) -- a LAN graph can exceed Python's recursion
    limit. Non-root ``u`` is a cut vertex if some DFS-child ``v`` has ``low[v] >= disc[u]``;
    the root is one iff it has >=2 DFS-tree children. Edge ``(u, v)`` is a bridge iff
    ``low[v] > disc[u]``."""
    disc: Dict[str, int] = {}
    low: Dict[str, int] = {}
    ...
    for start in adjacency:
        ...
        timer += 1
        stack = [(start, iter(adjacency.get(start, frozenset())))]
        while stack:
            node, it = stack[-1]
            descended = False
            for nbr in it:
                if nbr == parent[node]:
                    continue  # the single edge back to the parent, not a back edge
                if nbr not in disc:
                    parent[nbr] = node
                    ...
                    if node == start:
                        root_children += 1
                    stack.append((nbr, iter(adjacency.get(nbr, frozenset()))))
                    descended = True
                    break
            ...
            if low[node] > disc[up]:
                bridges.add(frozenset({up, node}))
            if up != start and low[node] >= disc[up]:
                points.add(up)
        if root_children > 1:
            points.add(start)
...
def find_articulation_points(graph: Graph) -> Set[str]:
    ...
    failure). Leaves and isolated nodes are never cut vertices."""
    return _tarjan(graph.adjacency)[0]
...
def find_bridges(graph: Graph) -> Set[FrozenSet[str]]:
    ...
    return _tarjan(graph.adjacency)[1]
