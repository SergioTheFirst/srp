...
def _add_neighbors(
    ...
    for snap in snapshots:
        for neighbor in snap.get("neighbors") or []:
            ...
            # D2: a neighbour's evidence is the ARP SNAPSHOT time, never the
            # agent's own heartbeat -- an agent can keep heartbeating for hours
            # after this particular ARP capture went stale. snapshot_at falls
            # back to last_seen only when a snapshot predates this column (old
            # net_snapshots rows / injected test fixtures without it).
            snapshot_at = snap.get("snapshot_at") or snap.get("last_seen")
            rec["last_seen"] = _newer(rec["last_seen"], snapshot_at)
            rec["sources"].add("arp")
