...
class LinkEvidence:
    ...
    # A9: which numbering space ``local_if`` lives in -- LLDP's local port number is
    # a bridge-port number, while FDB/CDP local_if is an ifTable ifIndex. The two
    # spaces are NOT the same numbers for the same physical port, so fusion must
    # never treat a coincidental digit match across spaces as a port conflict.
    port_space: str = "ifindex"
    ...
    for oid, value in session.walk(oids.LLDP_REM_CHASSIS_ID).items():
        ...
        out.append(
            LinkEvidence(
                ...
                a_port=a_port,
                b_port=b_port,
                port_space="lldp",  # lldpRemLocalPortNum is a bridge-port number, not ifIndex
            )
        )
    return out
...
def collect_evidence(
    ...
    *,
    ...
    radio_ifindexes: FrozenSet[int] = frozenset(),
) -> List[LinkEvidence]:
    ...
    tell an uplink from an edge. ``radio_ifindexes`` (A7) are this device's own
    radio ifIndexes, threaded straight to :func:`server.netdisco.l2.infer_edges`
    so an FDB edge off a radio port is tagged medium=wireless. The switch's own
    MACs are filtered automatically."""
    ...
    edges = l2.infer_edges(
        ...
        own_macs=_own_macs(device),
        radio_ifindexes=radio_ifindexes,
    )
