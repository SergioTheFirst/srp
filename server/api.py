...
from shared.schema import CONTRACT_VERSION, Envelope, is_contract_compatible, utcnow_iso

from server import db, limits, org_directory, updates
from server.analytics.diagnostics import compute_diagnostics
from server.analytics.health import apply_health_staleness
from server.ingest_guards import (
    check_rate_limit,
    count_reject,
    has_seen,
    mark_seen,
...
from server.netdisco.cache import _DEFAULT_TTL_SEC, GraphCache
from server.netdisco.collect import collect_status, start_collect
...
# Fire-and-forget worker threads (o5-B2) have no request to fail: they log instead.
_netlog = logging.getLogger("srp.netdisco")

router = APIRouter(prefix="/api/v1")


def _demo_guard() -> None:
    """Публичное демо (SRP_DEMO=1): read-only -- ни одна мутация не проходит.

    Обращаться ТОЛЬКО через ``limits.DEMO_MODE`` (не `from ... import DEMO_MODE`) --
    иначе monkeypatch в тестах не увидит подмену атрибута модуля."""
    if limits.DEMO_MODE:
        raise HTTPException(status_code=403, detail=limits.DEMO_READONLY_DETAIL)
...
def ingest(env: Envelope, request: Request) -> dict:
    ...
    # D2 (владелец): MAJOR-несовместимый агент останавливается ЗДЕСЬ, до записи --
    # раньше флаг уходил только в тело ответа, которое агент не читает (тихая
    # потеря контракта). Совместимость по MINOR/PATCH не проверяется (контракт
    # аддитивный) -- останавливаем только реальный разрыв контракта.
    if not is_contract_compatible(env.agent_version):
        count_reject("incompatible")
        raise HTTPException(
            status_code=406,
            detail=(
                f"agent_version {env.agent_version!r} incompatible with server "
                f"contract {CONTRACT_VERSION} (MAJOR mismatch)"
            ),
        )
    if (
        limits.MAX_DEVICES is not None
        and not db.device_known(env.device_id)
        and db.count_devices() >= limits.MAX_DEVICES
    ):
        count_reject("device_limit")
        raise HTTPException(status_code=403, detail=limits.limit_detail(limits.MAX_DEVICES))
    try:
...
def network_map_graph(request: Request, response: Response, at: Optional[str] = None) -> Any:
    ...
    o5-B8: the live branch is ETag-conditional. The ETag is a weak tag derived from
    the cache's ``loaded_at`` (unchanged while the cache is fresh), so a dashboard
    polling faster than the graph turns over can send ``If-None-Match`` and get a
    bodyless 304 instead of paying to re-serialize/re-send the whole graph."""
    ...
    # Graph and stamp come back from ONE call: reading the stamp separately would let
    # a background rebuild land in between and tag the graph we are about to send with
    # the newer graph's stamp -- a client caching that tag would then get 304s forever
    # and never see the update.
    graph, stamp = cache.get_with_stamp()
    value = graph or {}
    etag = f'W/"{int(stamp or 0)}"'
    if request.headers.get("if-none-match") == etag:
        return Response(status_code=304)
    response.headers["ETag"] = etag
    response.headers["Cache-Control"] = f"private, max-age={int(_DEFAULT_TTL_SEC)}"
    return value
...
async def poll_topology(request: Request) -> dict:
    ...
    o5-B2: a full cycle walks every known router/switch/AP over SNMP sequentially and
    can take a while, so the handler hands it to a worker thread and returns at once
    instead of blocking the dashboard's request on the walk."""
    _demo_guard()
    ...
    loop.run_in_executor(None, _run_topology_cycle_then_invalidate, cfg, request.app)
    return {"started": 1}


def _run_topology_cycle_then_invalidate(cfg: Any, app: Any) -> None:
    """Worker-thread body for the async force-poll (o5-B2).

    Runs the reconcile cycle to completion, THEN drops the unified-map cache -- in
    that order, so the next cache rebuild reads the cycle's own fresh
    net_links/snapshots rather than racing ahead of them (invalidating first, or
    invalidating from the request thread before the cycle writes anything, could
    let a concurrent reader's background rebuild win the race and re-cache a graph
    that is still missing the fresh edges until the TTL lapses on its own).
    ``run_topology_cycle``'s own ``_poll_lock`` still keeps a second concurrent
    cycle from actually running; a losing attempt here is just a no-op."""
    # Fire-and-forget: nobody awaits this future, so an escaping exception would be
    # swallowed silently -- the operator would see "started" and nothing would ever
    # happen. Log it here (the sync handler used to surface it as a 500) and still
    # invalidate, so a partial cycle's writes are not hidden behind a stale cache.
    try:
        netdisco_reconcile.run_topology_cycle(cfg)
    except Exception:  # noqa: BLE001 -- worker thread boundary: log, never propagate
        _netlog.exception("forced topology cycle failed")
    finally:
        cache = getattr(app.state, "network_map_cache", None)
...
@router.post("/network-map/collect")
def collect_network_map_now(request: Request) -> dict:
    """D1 "Собрать карту сейчас": everything the server can do without agents, one
    background job in order (discovery -> inventory -> classify -> topology ->
    reachability -> printers), ending in a SYNCHRONOUS cache rebuild so the very
    next dashboard read already shows the fresh graph -- unlike ``/topology/poll``,
    which only runs one cycle and leaves the cache to catch up on its own TTL.
    Unauthenticated, so rate-limited like every other force-poll button; each phase
    is still serialized by its own scheduler's anti-DoS lock."""
    _demo_guard()
    if not check_rate_limit("endpoint:network_map_collect"):
        raise HTTPException(status_code=429, detail="collect rate exceeded")
    cfg = getattr(request.app.state, "netdisco_config", None)
    if cfg is None:
        from server.netdisco.config import load_netdisco_config

        cfg = load_netdisco_config(None)
    # F5 (2026-08-23 review): the kill-switch is double-gated like main.py's own
    # loop-start check (app.state.netdisco_enabled AND cfg.enabled) -- an
    # operator who has netdisco off must not have this button start an active
    # scan anyway. Skipped cleanly, never a 4xx (the button is meant to degrade
    # gracefully, same spirit as the printer_poll_enabled gate below).
    if not getattr(request.app.state, "netdisco_enabled", False) or not cfg.enabled:
        return {"skipped": 1, "reason": "netdisco_disabled"}
    # printer_poll_enabled mirrors main.py's own gate for starting the periodic
    # printer loop (§ create_app lifespan) -- an operator who turned printer
    # polling off must not have this button probe printers anyway.
    printer_cfg = None
    if getattr(request.app.state, "printer_poll_enabled", False):
    ...
    cache = getattr(request.app.state, "network_map_cache", None) or GraphCache()
    return start_collect(cfg, printer_cfg, cache)


@router.get("/network-map/collect/status")
def network_map_collect_status() -> dict:
    """Poll target for the "собрать карту сейчас" button: running/phase/result."""
...
def fleet_print_export(
    ...
    # Самый дорогой read-эндпоинт (до _EXPORT_ROW_MAX строк в память и в ответ).
    # Токен намеренно НЕ требуется -- доверенная LAN, как у ack/meta (решение владельца).
    ...
    if len(rows) >= db._EXPORT_ROW_MAX:
        # Молчаливая обрезка учётного отчёта хуже отказа: оператор не отличит
        # полную выгрузку от урезанной. Помечаем и заголовком, и строкой в CSV.
        headers["X-SRP-Truncated"] = "1"
        # Полноценная CSV-строка, а не голый комментарий: строгий потребитель
        # (pandas без comment='#') иначе спотыкается о разное число полей.
        writer.writerow(
            {
                fieldnames[0]: f"# ВНИМАНИЕ: выгрузка обрезана на {db._EXPORT_ROW_MAX}"
                " строках (показаны самые свежие). Сузьте период или фильтр."
            }
        )
    buf.seek(0)
...
class MetaPatch(BaseModel):
    ...
    # F7 (o5 блок F): персональные данные владельца ПК, лимиты как в Envelope.
...
def patch_device_meta(device_id: str, body: MetaPatch) -> dict:
    ...
    if body.owner_full_name is not None:
        db.set_device_owner_full_name(device_id, body.owner_full_name)
    if body.owner_position is not None:
        db.set_device_owner_position(device_id, body.owner_position)
    if body.owner_phone is not None:
        db.set_device_owner_phone(device_id, body.owner_phone)
    return {"status": "ok"}
...
def printer_detail(printer_id: str, days: int = 30) -> dict:
    ...
    # review B3 MEDIUM: per-row supply history only feeds the dashboard's ETA
    # forecast (server/web/dashboard.py); this API never computes it, so it is
    # pure dead weight multiplying x500 rows in an unauthenticated response.
    for r in p.get("series") or []:
        r.pop("supplies", None)
    return p
...
def _require_admin_token(request: Request) -> None:
    """Проверка для админ-кнопок дашборда (удаление/очистка).

    Принимает ingest-токен ИЛИ отдельный admin_token. Второй существует затем,
    чтобы страница «Флот» могла отдать браузеру рабочий секрет, не публикуя
    ingest-ключ (он же запасной ключ подписи обновлений агента).
    """
    ingest = getattr(request.app.state, "ingest_token", "")
    admin = getattr(request.app.state, "admin_token", "")
    if not ingest and not admin:
        return  # аутентификация не настроена -- прежнее поведение (P0-2)
    provided = request.headers.get("x-srp-token") or ""
    for expected in (ingest, admin):
...
def _get_update_info_or_404(request: Request) -> dict:
    ...
    # U2-c: keyed per caller IP (not one shared fleet-wide bucket) so a single
    # noisy/misbehaving agent polling too fast can't starve every other agent's
    # update checks. Cardinality is bounded by the number of distinct LAN IPs
    # in the fleet -- the same _device_windows dict already opportunistically
    # trims past _TRIM_THRESHOLD (server/ingest_guards.py), same as the
    # per-device_id keys used elsewhere. Window/limit unchanged (30/60s).
    caller = request.client.host if request.client else "unknown"
    if not check_rate_limit(f"endpoint:agent_update:{caller}"):
        raise HTTPException(status_code=429, detail="rate limit exceeded")
