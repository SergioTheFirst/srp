...
def ingest_envelope(env: Envelope) -> dict[str, Any]:
    ...
    if env.msg_type == "inventory":
        ...
        db.upsert_device(
            ...
            comment=env.comment,
            owner_full_name=env.owner_full_name,
            owner_position=env.owner_position,
        ...
        # Узкая проекция сетевого среза: сборка карты читает её вместо полного JSON истории.
        db.store_net_snapshot(did, ts, env.payload)
        ...
        # available_version. D3: available_version -- это версия, которую ВИДИТ
        # агент, но не применяет (dev-сборка без секрета) -- отдельный сигнал от
        # серверного манифеста (server/updates.py); без хранения "ok" неотличим
        # от "ok, но не обновляется".
        db.touch_device(
            did,
            ts,
            env.agent_version,
        ...
        db.set_update_status(
            ...
            env.payload.get("available_version"),
        )
    ...
    return {
        ...
        # whether we consider it compatible. Since D2 a MAJOR mismatch is stopped
        # at the HTTP boundary (server/api.py, 406) before reaching this function,
        # so in practice this is always True here; field kept for transparency.
...
def recompute_scores(device_id: str) -> Optional[dict[str, Any]]:
    ...
    # o5-E1: доменные состояния для гейтов движков -- ровно как для сетевого ниже.
    # Имена доменов -- из server/trust/domains.py::DOMAIN_SOURCES.
    _domains = (trust or {}).get("domains") or {}
    storage_domain = _domains.get("storage") or {}
    disk_fill_domain = _domains.get("disk_fill") or {}
    os_domain = _domains.get("os_stability") or {}
    storage_risk = compute_storage_risk(
        ...
        domain_state=storage_domain.get("state"),
    )
    # Второе чтение журнала было строгим префиксом первого (get_recent_events
    # отдаёт новейшие первыми) -- режем срезом вместо повторного запроса.
    disk_fill_risk = compute_disk_fill_risk(
        hb_series,
        chain_events[:_TREND_HISTORY_LIMIT],
        device_trust=device_trust,
        domain_state=disk_fill_domain.get("state"),
    )
    os_degradation_risk = compute_os_degradation_risk(
        hist, device_trust=device_trust, domain_state=os_domain.get("state")
    )
